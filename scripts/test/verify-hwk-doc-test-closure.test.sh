...
test_document='docs/过程/测试/TST-DOC-06 HWK 作业与自动评测测试文档.md'
...
reject_text "$submission_component" '当前 CODE 结果读取路径（FR-HWK-04 缺口）'
require_text "$submission_component" '原子认领 PENDING 任务并标记 RUNNING'
require_text "$submission_component" '定时扫描 PENDING；启动重置遗留 RUNNING'
require_text "$submission_component" '只读最新状态'
reject_text "$publish_object" 'Question/TestCase Repository'
require_text "$publish_object" '保存 DRAFT 聚合（含题目、测试用例和评测配置）'
require_text "$publish_ssd" 'Homework + outbox 本地事务成功'
require_text "$publish_ssd" 'title、deadline、receiverScope'
reject_text "$publish_ssd" '必需通知投递成功'
require_text "$publish_object" 'HomeworkOutboxRepository'
require_text "$publish_object" '同一事务写入 homework.published.v2(title, deadline, receiverScope)'
reject_text "$publish_object" 'publishRequired(HOMEWORK_PUBLISHED)'
reject_text "$submission_object" '当前 CODE 结果读取路径（FR-HWK-04 缺口）'
require_text "$submission_object" 'HomeworkEvaluationTaskCreated'
require_text "$submission_object" '启动重置遗留 RUNNING'
require_text "$submission_object" '只读最新评测记录'
...
require_text "$closure" 'API-HWK-11 不再触发 evaluator'
require_text "$closure" 'GRD 来源成绩真实链路 | PASS'
require_text "$closure" '通知投递失败设计/实现一致性 | v1 历史 PASS，v2 不可接受'
require_text "$closure" 'Homework/outbox 失败返回 `503/HWK_5003`/DRAFT'
require_text "$closure" 'receiverScope=COURSE_ACTIVE_STUDENTS'
require_text "$closure" 'CODE 提交后独立后台评测'
require_text "$closure" 'FR-HWK-04：CODE 后台自动评测已由 #296 闭环'
require_text "$closure" '首次派发遗漏或进程重启后的恢复'
require_text "$closure" '#264 已由 PR #276 按文档与测试闭环交付完成'
require_text "$closure" 'publishRollsBackHomeworkWhenRequiredNotificationDeliveryFails'
require_text "$closure" '#281 / PR #285 已合并'
reject_text "$closure" '本 Issue 验收范围内无 FAIL 或 BLOCKED 项'
reject_text "$closure" '#264 完整闭环 PASS'
reject_text "$closure" '#264 不满足完整闭环或关闭条件'
reject_text "$closure" '通知投递失败设计/实现一致性 | FAIL'
reject_text "$closure" '通知投递失败设计/实现一致性 | PASS'
require_text "$test_document" '| 报告版本 | V2.0 |'
require_text "$test_document" '| 编写日期 | 2026-08-27 |'
require_text "$test_document" '| 最终结论 | #296 PASS |'
require_text "$test_document" 'TC-HWK-10/11 本地自动化判定 PASS'
require_text "$test_document" '| #264 共享 E2E | Playwright runner 原始计数：2 passed / 0 failed；'
require_text "$test_document" '| #296 自动化 | 后端相邻 105/105、全量 394'
require_text "$test_document" 'recoveryEvaluatesPersistedCodeSubmissionWhenInitialAfterCommitDispatchWasMissed'
require_text "$test_document" '| V2.1 | 2026-08-30 | D6 契约负责人 |'
require_text "$test_document" 'Homework + outbox 本地成功即 PUBLISHED'
reject_text "$test_document" '| 最终结论 | #264 FAIL |'
reject_text "$test_document" '| V1.7 | 2026-08-27 |'
reject_text "$test_document" '| #264 共享 E2E | 2/2 通过；'
require_text "$e2e" '@hwk'
require_text "$e2e" '教师创建发布作业后学生提交并获得已发布评语'
require_text "$e2e" '多类型提交验证代码后台评测'
require_text "$e2e" 'waitForSubmissionTerminal'
require_text "$e2e" "acceptedCodeResult.evaluationStatus).toBe('ACCEPTED')"
require_text "$e2e" "failedCodeResult.evaluationStatus).toBe('COMPILE_ERROR')"
reject_text "$e2e" '/evaluation'
