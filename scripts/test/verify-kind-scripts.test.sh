...
if ! env "${run_env[@]}" \
  GIT_SHA="$valid_sha" \
  KIND_SKIP_LOAD=1 \
  bash "$kind_scripts/kind-load-images.sh" >"$fixture_root/loadskip.out" 2>"$fixture_root/loadskip.err"; then
  fail "KIND_SKIP_LOAD=1 must succeed when images exist locally"
fi
[[ ! -s "$kind_log" ]] || fail "KIND_SKIP_LOAD=1 must not invoke kind load"
grep -q 'image present locally' "$fixture_root/loadskip.out" \
  || fail "KIND_SKIP_LOAD=1 must still verify local image presence"
pass "KIND_SKIP_LOAD=1 keeps local presence checks but skips kind load"
