...
if kindlib_kubectl get namespace "$K8S_NAMESPACE" >/dev/null 2>&1; then
  kindlib_note "deleting namespace '$K8S_NAMESPACE' (bounded wait, timeout $CLEANUP_TIMEOUT)"
  kindlib_kubectl delete namespace "$K8S_NAMESPACE" --wait=true --timeout="$CLEANUP_TIMEOUT"
else
