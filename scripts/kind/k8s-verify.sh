...
assert_pod_images() {
  ...
  [[ "$images" != *":latest"* ]] || kindlib_fail "$workload is running a latest-tagged image: $images"
  [[ "$images" == "$expected_image" ]] \
    || kindlib_fail "$workload runs '$images' but the contract requires exactly '$expected_image'"
  kindlib_note "PASS: $workload image = $expected_image"
}
...
assert_http_up() {
  ...
  local url="$2"
  local body
  if ! body="$(curl -fsS --retry 30 --retry-connrefused --retry-delay 1 --max-time 5 "$url")"; then
