...
# Escape hatch for environments where `kind load docker-image` cannot stream
# the tar (for example Docker Desktop's containerd image store exporting
# manifest lists with blobs the node's ctr rejects). The caller is then
# responsible for making the exact images above available on the node.
if [[ "${KIND_SKIP_LOAD:-0}" == "1" ]]; then
  kindlib_note "KIND_SKIP_LOAD=1: skipping kind load; all three images must already be present on the node"
  exit 0
fi
