...
kindlib_note "deploying mysql and waiting for bounded rollout (timeout $MYSQL_ROLLOUT_TIMEOUT_S)"
kindlib_kubectl apply -f "$render_dir/10-mysql-statefulset.yaml"
kindlib_kubectl apply -f "$render_dir/11-mysql-service.yaml"
kindlib_kubectl --namespace "$K8S_NAMESPACE" rollout status statefulset/mysql --timeout="$MYSQL_ROLLOUT_TIMEOUT_S"

kindlib_note "deploying backend and waiting for bounded rollout (timeout $BACKEND_ROLLOUT_TIMEOUT_S)"
kindlib_kubectl apply -f "$render_dir/20-backend-deployment.yaml"
kindlib_kubectl apply -f "$render_dir/21-backend-service.yaml"
kindlib_kubectl --namespace "$K8S_NAMESPACE" rollout status deployment/backend --timeout="$BACKEND_ROLLOUT_TIMEOUT_S"

kindlib_note "deploying frontend and waiting for bounded rollout (timeout $FRONTEND_ROLLOUT_TIMEOUT_S)"
...
kindlib_kubectl --namespace "$K8S_NAMESPACE" rollout status deployment/frontend --timeout="$FRONTEND_ROLLOUT_TIMEOUT_S"
