...
public final class TestJwtFactory {
    ...
    public static String userToken(KeyPair pair, String kid, String userId, List<String> roles) { return userToken(pair, kid, userId, roles, 1); }
    public static String userToken(KeyPair pair, String kid, String userId, List<String> roles, long securityVersion) { var claims = new LinkedHashMap<String, Object>(); long now = Instant.now().getEpochSecond(); claims.put("iss", "onlinejudge.identity.v2"); claims.put("aud", "onlinejudge.api"); claims.put("iat", now); claims.put("exp", now + 300); claims.put("userId", userId); claims.put("sessionId", "session-1"); claims.put("securityVersion", securityVersion); claims.put("roles", roles); claims.put("permissions", List.of()); return signed(pair, kid, claims); }
    public static String serviceToken(KeyPair pair, String kid, String audience, List<String> scopes) { var claims = new LinkedHashMap<String, Object>(); long now = Instant.now().getEpochSecond(); claims.put("iss", "onlinejudge.identity.v2"); claims.put("aud", audience); claims.put("iat", now); claims.put("exp", now + 300); claims.put("sub", "CN=grade-service"); claims.put("scopes", scopes); claims.put("jti", "service-token-1"); return signed(pair, kid, claims); }
