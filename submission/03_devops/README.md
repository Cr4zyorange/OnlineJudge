...
| CI 质量门禁 | `.github/workflows/ci.yml`、`scripts/ci/**` | #290；尚未合入当前 `dev` | 归档依赖、编译、测试、报告与失败诊断配置 |
| Kubernetes / Kind 基线 | `deploy/k8s/**`、`scripts/kind/**` | #288 PR #303；尚未合入当前 `dev` | 归档三服务清单、探针、部署和精确清理入口 |
| 端到端交付编排 | `.github/workflows/d3-delivery.yml`、`scripts/delivery/**` | #292；尚无 PR，尚未合入当前 `dev` | 归档临时 Kind、镜像加载、部署、诊断和清理流程 |
...
最终 D3 流水线在 GitHub-hosted Runner 上创建临时 Kind 集群，部署 `mysql`、`backend`、`frontend` 三项服务，完成健康和代理 API 验收后清理集群。它不依赖额外服务器、长期 Kubernetes 集群、生产域名、TLS 或云资源。当前 `origin/dev` 已包含 #287/#289；表中标记“尚未合入”的文件或入口不能据此宣称 Kind/Actions 已可运行。
...
由未参与实现的人在干净 checkout 执行 README 指向的实际脚本后，创建一次目录 `output/issue-291/replay-<UTC>/`。记录至少包括：
...
GitHub Actions 的成功、失败和测试报告以对应 workflow run 的 artifact 为唯一来源；#290/#292 合入前不预设 artifact 名称或伪造输出。macOS Docker Desktop、Linux GitHub-hosted Runner 和 Windows Git Bash/WSL 的差异由最终脚本吸收，或者必须在该次复演的 `environment.txt` 中明确记录。
