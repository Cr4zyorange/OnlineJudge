...
- `evidence/actions/historical/baseline-ce87/` 保留前一 final SHA 的 CI failure artifact 和 D3 outcome；`evidence/actions/historical/baseline-c56/` 保留更早 dev 基线的 CI 与 D3 原始 artifacts。同步后的 PR quality-gate artifact 仅保留 GitHub 在线清单，正式 final SHA 的成功 CI/D3 原始证据尚未归档。
...
静态计数回归失败；该问题已由 #388 修复并合入当前 `origin/dev`。同步后的 PR 候选
SHA 已由 run 33724384655 完成质量门禁和内置 Disposable delivery，但 `d3-delivery`
只接受 `dev` push；需要合入后产生合规的 final D3 run，当前不能写最终 `success=1`。
