...
本表以 2026-08-25 至 2026-09-03 GitHub PR 搜索、Issue/PR 元数据、正式 Review 和当日日报可复核的事项为准。它区分 PR Owner、共同提交者、Reviewer 与集成者；不能由提交数推断最终贡献比例，也不能代替成员确认。项目负责人于 2026-09-03 书面确认六人权重均为 1.0。

| 成员 | GitHub 标识 | 课程期 PR 索引，数量 | 代表 Issue 到 PR | 角色和 commit 事实 | 可执行或评审证据 | 权重 | 本人确认 |
| --- | --- | --- | --- | --- | --- | ---: | --- |
...
- 六项权重均为 1.0，总和 6.0：PASS（项目负责人确认）。
- 全部处于通常区间 0.9 至 1.1：PASS。
- 没有低于 0.9 的权重，因而不需要教师或助教特殊权重依据：PASS。
- 权重确认不是成员确认或签名：全员确认 0/6，确认门槛仍为 BLOCKED。
