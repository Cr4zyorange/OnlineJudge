...
[图 1-1 OnlineJudge Docker Compose 部署拓扑（模型源）](../models/arch/issue306-three-service-deployment.mmd)
...
### 5.9 Gateway 四上游入口、切流与回滚

浏览器和前端代码继续只访问 `/api`，但 Gateway 已从 frontend Nginx 中拆出，作为
`services/gateway/Dockerfile` 构建的独立 workload 运行。四个业务目标必须全部显式提供：
...
不支持独立 `LEARNING_UPSTREAM`、Learning/Grade 共用目标或 `backend:8080` 默认回退。
Identity 负责认证、当前用户、管理员接口和 `/.well-known/jwks.json`；Course 负责课程、章节、
学习过程、通知和提醒；Assessment 负责实验、作业、提交和评测；Grade 负责成绩项、记录、
发布、分析和复核。课程下 Assessment/Grade 子资源先于通用 Course 路由匹配。`/internal/v2/**`、
未知 `/api/**`、Worker、RabbitMQ、MySQL 和内部管理入口不对浏览器开放。

Gateway 关闭客户端请求 Header 的默认转发，再只重建公开契约允许的转发链、request ID、
Bearer、内容协商、内容长度/类型、Range 条件请求和 `Idempotency-Key`。因此任意
`X-User-*`、服务身份、内部 Token、hop-by-hop Header 和 `Connection` 声明的扩展 Header
都不会到达业务服务。业务服务必须继续本地验证 JWT、securityVersion、audience 和权限。

合法 `X-Request-Id` 原样透传，缺失或非法值由 Gateway 替换，并在响应返回最终值。普通
请求体上限 10 MB，Assessment 上传与提交为 55 MB；连接/普通读写/上传读写超时分别为
5/60/300 秒。Identity、查询、写入各有独立限流，超限返回 429。代理重试全局关闭，
避免非幂等请求被重复提交。Gateway 自有 404/413/429/502/503/504 响应包含稳定 code、
request ID 和 retryable，不暴露内部地址、堆栈或凭据；业务 401/403/404 等保持透传。

Compose overlay 为四个上游使用可覆盖默认值；切流脚本会将已验证的 `targets.env` 导出给
`docker compose`，重建后的 Gateway 因而消费本次切流目标。只验证配置合并时可执行：
...
```bash
export GATEWAY_BASE=http://127.0.0.1:8088
export GATEWAY_BEARER_TOKEN='<temporary-session-token>'
./scripts/gateway/switch-gateway-target.sh \
  --mode compose --service grade --target grade-canary:9084
```

服务名只允许 `identity`、`course`、`assessment`、`grade`，目标只允许小写 `host:port`。
状态文件固定包含 `identity`、`course`、`assessment`、`grade` 四个目标。脚本只改变
一个目标，重载独立 `gateway` 后检查健康和服务专属受保护 smoke。后置检查失败时恢复完整
前一快照并重复验证：成功退出 0；切换失败但回滚通过退出 1；回滚也无法验证退出 2。Kind
模式固定使用 `kind-onlinejudge-ci`、更新 `gateway-config` 并重启 #318 提供的
`deployment/gateway`，不再修改 frontend。
...
Docker Linux 引擎可用后运行 `bash scripts/gateway/tests/gateway-runtime.test.sh`，它创建四个
独立 upstream fixture 与 Gateway 镜像，覆盖 Identity JWKS、四上游路由、任意身份 Header、
request ID、深链、分页查询串、Range/下载、流式响应、multipart、401/403/404/413/429/
502/503/504、逐上游停机隔离和非幂等无重试。引擎不可用时明确退出 69。按 #317
`SCOPE_GATE_RESET`，四类 fixed stub 是本 issue 的验收正本；真实服务与浏览器主链由
#318/#320/#340 后续验收。
...
本次最终交付提供可由本地、GitHub Actions 和临时 Kubernetes 复用的固定镜像契约：构建方使用完整 `GIT_SHA` 执行 `scripts/docker/build-images.sh`，Compose 部署方以相同变量和固定镜像名执行 `scripts/docker/compose-images.sh up -d --no-build --wait --wait-timeout 240`，而 `scripts/delivery/run-kind-delivery.sh` 复用 #288 的 Kind 基线完成端到端验收。
> **D3 交付状态（2026-08-29）。** #287 数据库、#289 容器、#288（PR #303）Kind、#290（PR #298）质量门禁和 #292（PR #329）交付编排均已合入 `origin/dev@5cdbe8533991bb0c7cfbe23e08d81b78d47af483`。该精确 SHA 的目标仓库 [d3-delivery run 33227922081](https://github.com/Cr4zyorange/OnlineJudge/actions/runs/33227922081) 已成功。
