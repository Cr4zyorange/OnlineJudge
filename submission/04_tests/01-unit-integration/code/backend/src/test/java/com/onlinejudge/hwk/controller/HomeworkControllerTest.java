...
@Sql(
        statements = "CREATE TABLE IF NOT EXISTS assessment_event_outbox (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, event_id VARCHAR(64) NOT NULL UNIQUE, event_type VARCHAR(128) NOT NULL, payload_version INT NOT NULL, aggregate_type VARCHAR(64) NOT NULL, aggregate_id VARCHAR(64) NOT NULL, aggregate_version BIGINT NOT NULL, correlation_id VARCHAR(64) NOT NULL, payload_json CLOB NOT NULL, routing_key VARCHAR(192) NOT NULL, delivery_status VARCHAR(32) NOT NULL, attempt_count INT NOT NULL, next_attempt_at TIMESTAMP NOT NULL, lease_owner VARCHAR(128), lease_until TIMESTAMP, last_error VARCHAR(1024), published_at TIMESTAMP, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL)",
        executionPhase = Sql.ExecutionPhase.BEFORE_TEST_CLASS
)
@Sql(
        statements = {
...
class HomeworkControllerTest {
    ...
    @Test
    void teacherPublishesConfiguredHomeworkAndCommitsAssessmentOutboxWithoutSynchronousNotification() throws Exception {
        ...
        Integer outboxCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM assessment_event_outbox WHERE event_type = 'assessment.homework.published.v2' AND aggregate_id = ?",
                Integer.class, String.valueOf(homeworkId)
        );
        assertThat(outboxCount).isEqualTo(1);
        Map<String, Object> outbox = jdbcTemplate.queryForMap(
                "SELECT payload_json, correlation_id, delivery_status FROM assessment_event_outbox WHERE aggregate_id = ?",
                String.valueOf(homeworkId)
        );
        assertThat(outbox.get("delivery_status")).isEqualTo("PENDING");
        assertThat(outbox.get("correlation_id")).isNotNull();
        assertThat(String.valueOf(outbox.get("payload_json")))
                .contains("assessment.homework.published.v2")
                .contains("COURSE_ACTIVE_STUDENTS")
                .doesNotContain("7001")
    ...
    @Test
    void publishCommitsHomeworkAndOutboxWhenLegacySynchronousNotificationPublisherIsUnavailable() throws Exception {
        ...
        assertThat(notificationEventPublisher.events()).isEmpty();
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM assessment_event_outbox WHERE aggregate_id = ? AND delivery_status = 'PENDING'",
                Integer.class, String.valueOf(homeworkId)
        )).isEqualTo(1);
    }
