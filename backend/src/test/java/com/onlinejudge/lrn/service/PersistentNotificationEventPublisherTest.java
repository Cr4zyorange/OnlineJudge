...
class PersistentNotificationEventPublisherTest {
    ...
    void requiredPublishPropagatesPersistenceFailureToSourceTransaction() {
        ...
        PersistentNotificationEventPublisher publisher = publisher(notificationService);
    ...
    private PersistentNotificationEventPublisher publisher(NotificationService notificationService) {
        PlatformTransactionManager transactionManager = mock(PlatformTransactionManager.class);
        TransactionStatus transactionStatus = mock(TransactionStatus.class);
        when(transactionManager.getTransaction(any())).thenReturn(transactionStatus);
        Executor directExecutor = Runnable::run;
        return new PersistentNotificationEventPublisher(
                notificationService,
                transactionManager,
                directExecutor
        );
