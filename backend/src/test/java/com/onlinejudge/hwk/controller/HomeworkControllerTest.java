...
class HomeworkControllerTest {
    ...
    @Test
    void submissionHistoryMarksLatestAndTeacherCanListFilterAndViewDetails() throws Exception {
        long homeworkId = createHomeworkAndReturnId(Map.ofEntries(
                entry("courseId", 101),
                entry("chapterId", 11),
                entry("title", "HWK03 history text"),
                entry("description", "Keep every submitted version."),
                entry("type", "TEXT"),
                entry("deadline", "2026-06-30T23:59:59"),
                entry("totalScore", 100),
                entry("allowResubmit", true),
        ...
        long firstSubmissionId = submitTextAndReturnId(homeworkId, "first version");
        long secondSubmissionId = submitTextAndReturnId(homeworkId, "second version");
        ...
        mockMvc.perform(get("/api/v1/homeworks/{homeworkId}/submissions", homeworkId)
                ...
                .andExpect(jsonPath("$.data.list", hasSize(1)))
                .andExpect(jsonPath("$.data.list[0].id").value(secondSubmissionId))
                .andExpect(jsonPath("$.data.list[0].studentId").value(601))
                .andExpect(jsonPath("$.data.list[0].latest").value(true))
                .andExpect(jsonPath("$.data.list[0].final").value(true));

        mockMvc.perform(get("/api/v1/submissions/{submissionId}", firstSubmissionId)
                        .headers(studentHeaders("101")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.answerText").value("first version"))
                .andExpect(jsonPath("$.data.latest").value(false))
        ...
        mockMvc.perform(get("/api/v1/submissions/{submissionId}", secondSubmissionId)
                        .headers(studentHeaders("777", "101")))
                .andExpect(status().isForbidden());
    }
    ...
    private long submitTextAndReturnId(long homeworkId, String answerText) throws Exception {
    ...
    private org.springframework.http.HttpHeaders studentHeaders(String courseIds) {
        return studentHeaders("601", courseIds);
    }

    private org.springframework.http.HttpHeaders studentHeaders(String userId, String courseIds) {
        org.springframework.http.HttpHeaders headers = new org.springframework.http.HttpHeaders();
