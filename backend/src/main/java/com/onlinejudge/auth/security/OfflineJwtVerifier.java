...
/**
 * Frozen v2 user-token protocol verifier. It deliberately has neither an HTTP client nor an
 * Identity database dependency: every business request uses only a previously cached JWKS and
 * this service's security-version projection.
 */
public final class OfflineJwtVerifier {
    ...
    private OfflineJwtVerifier(ObjectMapper objectMapper, Clock clock, String issuer, String audience, Map<String, PublicKey> publicKeys) {
    ...
    public static OfflineJwtVerifier fromJwks(ObjectMapper objectMapper, Clock clock, String issuer, String audience, Map<String, Object> jwks) {
    ...
    public Verification verify(String token, SecurityVersionLookup securityVersions) {
        try {
            ...
            return new Verification(true, null, new Principal(
                    userId,
    ...
    private static Map<String, PublicKey> parseJwks(Map<String, Object> jwks) {
        ...
        for (Object item : keyList) {
            ...
            if (!"RSA".equals(stringValue(raw.get("kty")))
                    ...
                    || !ALGORITHM.equals(stringValue(raw.get("alg")))) {
    ...
    public enum Rejection {
        MALFORMED, ALGORITHM_NOT_ALLOWED, UNKNOWN_KID, SIGNATURE_INVALID, ISSUER_MISMATCH,
        AUDIENCE_MISMATCH, ISSUED_IN_FUTURE, EXPIRED, SECURITY_VERSION_STALE
    }

    public record Verification(boolean accepted, Rejection rejection, Principal principal) { }

    public record Principal(String userId, String sessionId, long securityVersion, List<String> roles,
                            List<String> permissions, Instant issuedAt, Instant expiresAt) { }
}
