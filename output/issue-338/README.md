...
| RED（最终 P1：token direction + source-grade 条件） | 在 `e98f18805439d8b1982e489debada5e852b0de1d` 先加入 response/typed-payload 语义门禁及真实未评分事件 fixture，尚未改 JSON 制品 | 预期失败：14 problems，exit 1；`accessToken` 仍为 `writeOnly`，SourceGrade/Event 未声明 `status`/互斥分支，实际 `UNGRADED + score:null` 合法 fixture 被拒绝 | 本次 PR 终端原始输出；生产者事实由 `SourceGradeProducerContractTest#labProviderMarksUngradedSubmissionWithoutFabricatingScore` 给出 |
| GREEN（最终 P1：token direction + source-grade 条件） | `node scripts/ci/verify-microservice-contract-v2.mjs`；Java 21 `mvn -Dtest=SourceGradeProducerContractTest test` | 5 OpenAPI、8 AsyncAPI、3 valid、7 incompatible rejected、14 rejecting mutations rejected；真实生产者 5/5 | 本次 PR 终端原始输出 |
...
最终 P1 的 v2 正本现以同一不可弱化条件收敛：`ServiceTokenResponse.accessToken` 是 closed `201` response 的必填 `readOnly`/response-only 字段，`ServiceTokenRequest` 不接收 token；Assessment `SourceGrade` REST 和 `assessment.source-grade.changed.v2` 都要求 `status`，并用互斥分支规定 `SCORED → number`、`UNGRADED → null`。反例覆盖 SCORED/null、UNGRADED/number、移除 event status、将 token 改回 writeOnly、以及请求体 token 泄漏；D6、ADR-006、最终 SAD/DSD 的人类契约由同一 verifier 与文档 mutation 闭环检查。
