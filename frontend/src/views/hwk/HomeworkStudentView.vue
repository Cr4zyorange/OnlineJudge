      ...
      <section class="student-homework__history" aria-label="我的提交">
        ...
        <ul v-else>
          <li v-for="submission in submissions" :key="submission.id">
            ...
            <span v-if="submission.latest" class="student-homework__badge">最新提交</span>
            <span v-if="submission.final" class="student-homework__badge">当前有效</span>
        ...
        <section v-if="selectedSubmission" class="student-homework__submission-detail" aria-label="提交详情">
          <h3>提交详情</h3>
          <p>状态：{{ selectedSubmission.submitStatus }}</p>
          <p>提交时间：{{ formatDate(selectedSubmission.submittedAt) }}</p>
...
async function openSubmission(submissionId: number) {
  ...
  try {
    ...
    selectedSubmission.value = null;
    historyError.value = error instanceof Error ? error.message : '提交详情加载失败';
  }
}
