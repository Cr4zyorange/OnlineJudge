...
export function resolveAuthContext(locationLike: Pick<Location, 'search' | 'pathname'>): AuthContext {
  ...
  const userId = params.get('userId') ?? window.localStorage.getItem('onlinejudge.userId') ?? '101';
  const username = params.get('username') ?? window.localStorage.getItem('onlinejudge.username') ?? 'Teacher101';
  const role = params.get('role') ?? window.localStorage.getItem('onlinejudge.userRole') ?? 'TEACHER';
