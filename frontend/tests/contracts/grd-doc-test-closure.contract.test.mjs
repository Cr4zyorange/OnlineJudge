...
test('GRD SSD branches and responses match the implemented API contracts', () => {
  ...
  assert.match(gradeReviewSsd, /来源事务提交后.*通知.*独立事务/);
  assert.match(gradeReviewSsd, /来源事务回滚.*不生成通知/);
  ...
  assert.match(gradeReviewUseCase, /after-commit.*独立事务/);
  assert.match(gradeTestReport, /#283.*PR #284.*独立/);
  assert.doesNotMatch(notificationPublisher, /@Async/);
  assert.doesNotMatch(notificationPublisher, /NOT_SUPPORTED/);
  ...
  assert.match(gradePublishUseCase, /after-commit.*独立事务/);
  assert.match(gradePublishUseCase, /notificationStatus.*SENT.*不能证明.*通知.*落库/);
  assert.match(gradeFlowSsd, /相同发布范围已成功[\s\S]*幂等返回既有 publishId[\s\S]*不重复.*通知/);
  assert.match(gradeFlowSsd, /notificationStatus=SENT/);
  assert.match(gradeFlowSsd, /after-commit.*独立事务/);
