...
describe('homeworks api', () => {
  ...
  it('builds documented HWK01 routes and unwraps ApiResponse data', async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(jsonResponse([submission({ id: 31 })]))
      .mockResolvedValueOnce(jsonResponse({ list: [submission({ id: 32 })], page: 1, size: 20, total: 1 }))
      .mockResolvedValueOnce(jsonResponse(submission({ id: 32 })))
    ...
    await listHomeworkSubmissions(11, { submitStatus: 'SUBMITTED', page: 1, size: 20 });
    await getHomeworkSubmission(32);
    ...
    expect(fetchMock.mock.calls.map((call) => [call[0], (call[1] as RequestInit).method])).toEqual([
      ...
      ['/api/v1/homeworks/11/submissions?page=1&size=20&submitStatus=SUBMITTED', 'GET'],
      ['/api/v1/submissions/32', 'GET'],
