...
describe('HomeworkStudentView', () => {
  ...
  it('marks the latest and current effective submission in student history', async () => {
    vi.mocked(homeworkApi.listHomeworks).mockResolvedValueOnce({
      list: [homeworkSummary()],
      page: 1,
      size: 20,
  ...
  it('opens a selected submission detail from student history', async () => {
    vi.mocked(homeworkApi.listHomeworks).mockResolvedValueOnce({
      list: [homeworkSummary()],
      page: 1,
      size: 20,
    ...
    vi.mocked(homeworkApi.getHomeworkDetail).mockResolvedValueOnce(homeworkDetail());
    vi.mocked(homeworkApi.listMyHomeworkSubmissions).mockResolvedValueOnce([
      submission({ id: 92, answerText: 'history answer', final: true, latest: true })
    ]);
    vi.mocked(homeworkApi.getHomeworkSubmission).mockResolvedValueOnce(
      submission({ id: 92, answerText: 'detail answer', final: true, latest: true })
    );
    ...
    expect(wrapper.text()).toContain('detail answer');
  });
});
