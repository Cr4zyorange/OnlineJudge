...
`node scripts/ci/verify-microservice-contract-v2.mjs` 校验五份 OpenAPI 的 service-identity 401/403 语义、AsyncAPI 的 typed-envelope/生产消费目录/ordering、v1 历史指向以及合法与不兼容事件样例；它还验证删除 ordering、清空 payload、篡改 aggregate 的 mutation 必被拒绝。该检查仅使用 Node 标准库，适用于干净环境。
