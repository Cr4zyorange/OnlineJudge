...
| 测试结论 | GRD 单元/接口/迁移/集成测试及真实 LAB/HWK → GRD → LRN API E2E 闭环通过；页面视觉验收和生产级性能压测仍需专项确认 |
...
| V1.8 | 2026-08-25 | GRD 模块负责人 | 按 PR #270 七轮评审隔离外部 Spring/Compose 环境，并将复核通知对齐为同步 best-effort 持久化 |
| V1.9 | 2026-08-26 | GRD 模块负责人 | 按 PR #270 十六轮评审隔离 LRN 通知与 GRD 外层事务，防止通知回滚标记导致发布/复核提交抛出 `UnexpectedRollbackException` |
...
| FR-GR-04 | 成绩发布与状态控制 | 发布前检查、发布记录、发布后学生可见、相同范围重复发布返回同一 `publishId`、发布通知同步 best-effort 语义、发布后调整留痕 | 后端、前端和真实 API E2E 已覆盖；publisher 挂起外层 GRD 事务，LRN 独立事务失败只记告警，发布记录及响应中的 `notificationStatus` 仍为 `SENT` |
...
| FR-GR-07 | 成绩异议与复核申请 | 学生提交异议、重复申请拦截、教师同意/驳回、复用调整留痕和通知 | 后端、前端和真实 API E2E 已覆盖；通知在响应前使用独立事务同步 best-effort 持久化，失败只回滚通知侧并记日志，不改变申请与响应 |
| NFR-GR-01 | 可靠性 | 来源同步、重算、发布、通知失败和事务边界保持数据一致 | 自动化覆盖来源超时、来源删除、缺失、重复同步/发布和核心事务边界；来源提供方未映射异常在服务层整体回滚，API 当前返回 HTTP 500、`code=500` 和“系统错误，请联系管理员”；发布/复核通知由 publisher 挂起 GRD 外层事务，LRN 独立事务持久化异常只影响通知侧，不将 GRD 事务标记为 rollback-only；当前 `notificationStatus` 仍为 `SENT`，没有 `FAILED` 回写或补偿状态 |
...
| GRD-266-LOG-020 | PR #270 十六轮通知事务隔离回归 | 真实 Spring/H2 事务红测稳定复现 `UnexpectedRollbackException`；publisher 改为 `NOT_SUPPORTED` 挂起外层事务后目标绿测 1 passed / 0 failed，通知事务回滚且外层业务记录成功提交；发布/复核/LRN 相关后端回归 24 passed / 0 failed，文档/E2E 契约 8 passed / 0 failed，前端类型检查、四项 shell 契约、SVG XML 和 `git diff --check` 通过 |
