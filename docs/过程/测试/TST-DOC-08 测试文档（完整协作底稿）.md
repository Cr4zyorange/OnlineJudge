...
| 测试结论 | 有条件通过；后端/前端自动化、本地真实服务闭环探针、本地安全探针和 Docker Python 基础专项通过，生产级安全扫描、生产实流量长稳和最终人工审批仍需验收环境确认 |
...
| 范围项 | 说明 | 处理方式 |
| --- | --- | --- |
| 完整验收环境浏览器端到端 | 已执行本地 DEV 真实服务闭环探针；FAT/UAT 环境仍需按验收账号补截图或测试记录 | 最终验收补充 |
...
| TC-LN-02-07 | FR-LN-02 | CRS/LAB/HWK 页面继续学习恢复断点 | 已存在 `lastPosition` | CRS `chapterId`、LAB/HWK `resume` 查询参数 | 从学习进度页点击继续学习 | 目标页面按断点恢复章节、代码或作答上下文 | 前端单元测试通过；真实浏览器待手工确认 | 有条件通过 | `CourseManagementView.spec.ts`、`LabStudentView.spec.ts`、`HomeworkStudentView.spec.ts` |
...
| TC-LN-05-03 | FR-LN-05 | 通知业务跳转 | 用户已登录且通知含 `actionUrl` | 指向任务、成绩、公告或系统页面的通知 | 在通知中心点击查看/跳转 | 跳转到对应业务页面，通知内容不丢失 | 前端组件测试覆盖链接渲染；真实浏览器待手工确认 | 有条件通过 | `NotificationCenterView.spec.ts` |
...
| --- | --- | --- | --- | --- | --- |
| MAN-AUTH-001 | AUTH | 浏览器登录、刷新和退出 | 使用学生、教师、管理员账号登录，刷新页面，退出后访问 `/courses` 等需认证页面 | 登录态保持；退出后跳转登录或提示会话失效 | 有条件通过；2026-06-12 本地浏览器验证学生登录成功和过期会话提示，教师/管理员完整浏览器矩阵待 FAT/UAT 复验 |
| MAN-AUTH-002 | AUTH | 浏览器注册和失败提示 | 切换注册模式，创建学生账号；提交重复邮箱、弱密码或缺失字段 | 成功后回到登录；失败提示清晰且不暴露内部细节 | 待手工验收；已有前端单测覆盖注册模式和失败提示 |
| MAN-AUTH-003 | AUTH | 个人资料与修改密码 | 登录后进入个人资料页，修改昵称/联系方式/头像，修改密码后使用旧密码和旧会话访问 | 资料更新；旧密码和旧会话失效；提示明确 | 待手工验收；已有后端和前端单测覆盖 |
| MAN-AUTH-004 | AUTH | 管理员用户、角色、权限管理 | 管理员查询用户、创建用户、调整角色、调整角色权限、禁用账号 | 管理操作成功；目标用户权限刷新；禁用账号无法继续访问 | 待手工验收；本地 HTTP 探针已验证管理员用户列表可访问 |
...
| MAN-AUTH-006 | AUTH | 会话过期和账号异常状态 | 手工制造过期 token、撤销 token、锁定账号、禁用账号 | 前端清理登录态并跳转登录失效或账号状态页 | 有条件通过；2026-06-12 浏览器 smoke 观察到过期会话页，账号异常矩阵待 FAT/UAT 复验 |
| MAN-AUTH-007 | AUTH/CRS/HWK/LAB/GRD/LRN | 主流程登录态联调 | 登录后进入课程、作业/实验、成绩、通知页面，确认各模块使用后端当前用户上下文 | 不需要前端传操作者 `userId`；各模块继续执行业务归属校验 | 有条件通过；2026-06-12 干净 H2 后端 HTTP 闭环探针通过，FAT/UAT 真实环境待复核 |
...
| MAN-AUTH-010 | AUTH/全部模块 | 统一测试环境闭环 | 执行“登录 -> 课程 -> 作业/实验 -> 提交/评测 -> 成绩 -> 通知” | AUTH 登录态贯穿主流程，异常和权限边界一致 | 有条件通过；2026-06-12 干净 H2 后端闭环探针和浏览器 smoke 通过，最终验收环境待确认 |
...
| --- | --- | --- | --- | --- | --- | --- |
| LRN-MAN-001 | TC-LN-01 | 首页进入学习任务中心并查看分页 | 本地前后端启动，学生账号已登录并加入课程 | 从首页点击学习任务入口，切换状态/类型筛选，点击上一页/下一页 | 页面毛玻璃风格一致；任务为当前学生可见课程；分页可切换；空态/失败态清晰 | 有条件通过；2026-06-12 浏览器 smoke 可见学习任务中心、实验、作业和资源任务 |
| LRN-MAN-002 | TC-LN-02 | 课程资源断点续传 | 学生已打开课程资源并产生进度 | 从学习进度页点击继续学习 | 回到课程页对应章节/资源位置，不进入错误课程 | 有条件通过；前端单测和干净 H2 HTTP 探针覆盖断点数据，真实点击恢复待 FAT/UAT 复验 |
| LRN-MAN-003 | TC-LN-02 | LAB/HWK 断点续传 | 学生在实验/作业中保存草稿或产生进度 | 从学习进度页点击继续学习到 LAB/HWK | 代码或作答上下文恢复，打开页面不会立即覆盖原断点 | 有条件通过；前端单测覆盖，完整浏览器交互待 FAT/UAT 复验 |
| LRN-MAN-004 | TC-LN-03 | 仪表盘数据随真实学习动作变化 | 学生进入 CRS/LAB/HWK 并完成访问或提交 | 打开学习仪表盘，观察总时长、访问次数、趋势和最近记录 | 统计与真实操作一致，失败时展示重试或缓存提示 | 有条件通过；HTTP 探针验证统计接口，真实操作计时待 FAT/UAT 复验 |
| LRN-MAN-005 | TC-LN-04、TC-LN-05 | 通知中心分类、已读、删除、跳转 | 账号存在任务、成绩、公告、系统通知 | 切换类型/未读筛选，批量已读，删除单条，点击业务跳转 | 未读数变化正确；删除后列表隐藏；跳转到对应业务页；其他用户通知不受影响 | 有条件通过；2026-06-12 浏览器 smoke 可见通知中心、LAB/HWK/GRD 通知，批量操作待 FAT/UAT 复验 |
| LRN-MAN-006 | TC-LN-04 | 跨模块真实事件生成通知 | CRS/LAB/HWK/GRD 模块在同一环境可用 | 发布公告、发布实验/作业、发布成绩或复核成绩 | LRN 收到事件并生成分类通知，重复操作不生成重复通知 | 有条件通过；干净 H2 演示数据和集成测试验证结果通知，真实事件触发链待 FAT/UAT 复验 |
| LRN-MAN-007 | TC-LN-06 | 提醒规则设置和截止提醒 | 学生有临近截止且未提交的 LAB/HWK | 修改提醒偏好，等待或触发提醒扫描 | 符合规则的未提交任务产生提醒；关闭非必要提醒后不再收到非必要提醒 | 有条件通过；后端/前端单测覆盖，真实调度待 FAT/UAT 复验 |
| LRN-MAN-008 | TC-LN-N01 | 权限边界手工核对 | 准备学生 A、学生 B、教师、非成员账号 | 分别访问学习任务、进度、仪表盘、通知和提醒页面 | A/B 互不泄露数据；非成员被拒绝；学生不能查看教师聚合 | 有条件通过；自动化和本地安全探针覆盖核心边界，完整账号矩阵待复验 |
| LRN-MAN-009 | TC-LN-N01、TC-LN-N04 | 会话过期和网络异常 | 删除 token 或关闭后端服务 | 打开 LRN 页面并执行刷新/保存/已读/删除操作 | 显示登录失效或网络失败提示，不展示其他用户缓存 | 有条件通过；浏览器 smoke 覆盖过期会话页，网络异常矩阵待复验 |
| LRN-MAN-010 | TC-LN-N02、TC-LN-N03 | 通知触达时延和可靠性 | 测试环境支持通知轮询或推送 | 触发任务/成绩/公告通知并计时 | 通知列表和未读数在设计阈值内刷新，断线恢复后不丢通知 | 待联调确认；需在 FAT/UAT 环境补触达计时记录 |

### 9.4 LAB 手工测试与联调确认

| 手测编号 | 模块 | 场景 | 操作要点 | 预期结果 | 当前结果 |
| --- | --- | --- | --- | --- | --- |
| MAN-LAB-001 | LAB/AUTH/CRS | 教师创建并发布实验 | 浏览器登录教师账号，进入课程，创建实验并发布 | 实验保存成功，课程学生可见，教师列表状态刷新 | 有条件通过；自动化覆盖，教师浏览器创建发布待 FAT/UAT 复验 |
| MAN-LAB-002 | LAB | 学生查看实验并提交代码 | 学生进入实验详情，选择语言并提交代码 | 提交成功，显示受理状态并最终刷新评测结果 | 有条件通过；2026-06-12 浏览器任务页和干净 H2 HTTP 探针验证实验可见，真实提交待 FAT/UAT 复验 |
| MAN-LAB-003 | LAB | 学生上传实验报告 | 学生在已有提交基础上上传 PDF/DOCX/ZIP 报告 | 报告上传成功，最新版本正确，下载入口可用 | 有条件通过；自动化覆盖，真实文件环境待验收 |
| MAN-LAB-004 | LAB | 教师查看提交并评分 | 教师筛选学生提交，查看代码、报告、评测结果并评分 | 自动分、报告分、人工分、最终分和评语保存成功 | 有条件通过；自动化覆盖，教师浏览器操作待 FAT/UAT 复验 |
| MAN-LAB-005 | LAB | 学生查看发布前后结果差异 | 成绩发布前后分别进入结果页 | 发布前隐藏教师评分与报告评分，发布后展示完整反馈 | 有条件通过；自动化覆盖发布前后差异，浏览器详情待复验 |
| MAN-LAB-006 | LAB | 教师查看实验统计 | 打开统计页查看提交率、未提交名单和分数分布 | 统计数据与实际样本一致，页面可视化显示正常 | 有条件通过；自动化覆盖统计接口和组件，浏览器详情待复验 |
| MAN-LAB-007 | LAB | 权限与异常边界 | 非成员、学生访问教师入口、他人结果访问、实验已截止重提 | 页面提示明确，接口返回受控错误 | 有条件通过；自动化覆盖核心边界，完整浏览器矩阵待复验 |
| MAN-LAB-008 | LAB | 真实 Docker 沙箱专项测试 | 提交 AC/WA/编译错误/运行错误/超时/内存超限样本，多语言并发执行 | 状态、日志、资源限制和超时控制符合设计 | 通过；2026-06-12 `OJ_DOCKER_SANDBOX_TEST=true mvn -Dtest=DockerSandboxExecutorTest test` 复跑 2 条通过；Java 多语言端到端仍需验收环境确认 |
| MAN-LAB-009 | LAB | 基础性能样本 | 准备更多课程学生和实验提交，观察提交受理与统计接口响应时间 | 提交快速受理，统计查询在设计阈值内返回 | 本地压测通过 |
| MAN-LAB-010 | LAB/LRN/GRD | 跨模块联调 | 发布实验、提交、评分、发布成绩后检查通知中心和成绩同步 | LRN 能看到实验发布/成绩发布通知，GRD 能读取 LAB 来源成绩 | 有条件通过；干净 H2 HTTP 探针验证 LAB 通知和 GRD 成绩链路，真实事件触发待 FAT/UAT 复验 |

### 9.5 HWK 手工测试与联调确认

| 手测编号 | 模块 | 场景 | 操作要点 | 预期结果 | 当前结果 |
| --- | --- | --- | --- | --- | --- |
| MAN-HWK-001 | HWK/AUTH/CRS | 教师创建并发布作业 | 浏览器登录教师账号，进入课程，创建文本/客观题/代码题作业并发布 | 作业保存成功，学生可见，教师端状态刷新 | 有条件通过；自动化覆盖，教师浏览器创建发布待 FAT/UAT 复验 |
| MAN-HWK-002 | HWK | 学生提交作业 | 学生登录后进入作业详情，提交文本答案、客观题答案和代码答案 | 提交成功，显示提交时间和初始评测状态 | 有条件通过；2026-06-12 浏览器任务页和干净 H2 HTTP 探针验证作业可见与评测结果，真实提交待 FAT/UAT 复验 |
| MAN-HWK-003 | HWK | 教师批阅与重评 | 教师查看提交列表，筛选学生，录入人工分数/评语，触发重评 | 分数、评语、评测状态和日志刷新 | 有条件通过；自动化覆盖，教师浏览器操作待 FAT/UAT 复验 |
| MAN-HWK-004 | HWK | 学生查看反馈 | 成绩发布前后分别查看详情、历史和评测结果 | 发布前隐藏最终分，发布后展示允许公开的反馈 | 有条件通过；干净 H2 HTTP 探针验证已发布评测结果，浏览器详情待复验 |
| MAN-HWK-005 | HWK | 权限边界 | 非成员、其他学生、无课程管理权限教师访问 HWK 页面和接口 | 页面提示权限不足，接口返回受控错误，不泄露敏感字段 | 有条件通过；自动化覆盖核心边界，完整浏览器矩阵待复验 |
| MAN-HWK-006 | HWK | 页面状态 | 人为制造加载中、空列表、接口失败、会话过期、发布配置不完整 | 页面有清晰提示，操作入口禁用或引导正确 | 有条件通过；前端单测覆盖，FAT/UAT 视觉复验待补 |
| MAN-HWK-007 | HWK/LAB | 真实代码评测沙箱 | 使用真实 Docker 沙箱提交 Python/Java 代码，包含 AC、WA、编译错误、超时 | 状态、日志、资源限制和隐藏用例显示策略正确 | 有条件通过；真实 Docker Python 执行器测试通过，HWK 真实页面提交和 Java 样本待验收环境确认 |
| MAN-HWK-008 | HWK | 基础性能 | 准备大批量作业、提交和未提交学生，查询列表/统计 | 分页正常，响应时间满足测试负责人设定阈值 | 本地压测通过 |
| MAN-HWK-009 | HWK/LRN/GRD | 跨模块联调 | 发布作业、完成评测/批阅、发布成绩，查看通知中心、学习任务、成绩同步 | LRN 通知/提醒生成，GRD 可同步 HWK 来源成绩 | 有条件通过；干净 H2 HTTP 探针验证 HWK 通知、评测和 GRD 成绩链路，真实事件触发待 FAT/UAT 复验 |

### 9.6 GRD 手工测试与联调确认

| 手测编号 | 模块 | 场景 | 操作要点 | 预期结果 | 当前结果 |
| --- | --- | --- | --- | --- | --- |
| MAN-GRD-001 | GRD/AUTH/CRS | 教师配置成绩项 | 浏览器登录教师账号，进入课程成绩项配置页，创建 LAB/HWK 来源成绩项并校验权重 | 成绩项保存成功，非法权重和非法来源提示明确 | 有条件通过；自动化覆盖，教师浏览器配置待 FAT/UAT 复验 |
| MAN-GRD-002 | GRD/LAB/HWK | 同步来源成绩并计算总评 | 准备真实实验/作业评分，教师触发同步和重算 | 成绩记录、加权分、缺失状态、总评与 LAB/HWK 来源一致 | 有条件通过；干净 H2 HTTP 探针验证总评 89.6 和 LAB/HWK 来源链，真实同步按钮待复验 |
| MAN-GRD-003 | GRD | 教师成绩总表与学生明细 | 教师筛选分页查看成绩总表，打开学生明细，查看来源任务和状态 | 总表分页、筛选、明细、缺失状态和来源信息正确 | 有条件通过；干净 H2 HTTP 探针验证教师成绩总表，浏览器详情待复验 |
| MAN-GRD-004 | GRD/LRN | 成绩发布与学生可见 | 教师发布成绩，学生刷新个人成绩页，通知中心查看成绩发布通知 | 发布记录保存，学生只能看到本人已发布成绩，LRN 通知可见 | 有条件通过；干净 H2 HTTP 探针和浏览器通知页验证已发布成绩与 GRD 通知，真实发布操作待复验 |
| MAN-GRD-005 | GRD | 成绩调整与变更记录 | 教师对已发布单项成绩或总评进行带原因调整 | 分数更新，变更记录显示旧值、新值、原因、操作人和时间 | 有条件通过；自动化覆盖，浏览器操作待 FAT/UAT 复验 |
| MAN-GRD-006 | GRD | 教学分析 | 教师查看课程总评分析和单项成绩完成情况 | 均分、最高分、最低分、及格率、完成率、分布和来源时间点正确 | 有条件通过；自动化覆盖，浏览器页面待复验 |
| MAN-GRD-007 | GRD/LRN | 成绩异议复核 | 学生对已发布成绩提交异议，教师处理同意或驳回，学生查看结果 | 申请状态流转正确，重复申请被拦截，处理结果通知可见 | 有条件通过；自动化覆盖，浏览器完整操作待复验 |
| MAN-GRD-008 | GRD/AUTH/CRS | 权限边界 | 非课程教师、非成员学生、学生访问教师接口、教师访问学生个人接口 | 页面提示权限不足，接口返回受控错误，不泄露成绩数据 | 有条件通过；自动化和本地安全探针覆盖核心边界，完整账号矩阵待复验 |
| MAN-GRD-009 | GRD | 页面状态 | 制造加载中、空成绩项、无成绩记录、接口失败、会话过期 | 页面有清晰提示，按钮禁用或引导正确 | 有条件通过；前端单测覆盖，FAT/UAT 视觉复验待补 |
...
| TST08-RISK-003 | FAT/UAT 浏览器矩阵未完整留痕 | 2026-06-12 已在本地浏览器验证过期会话、学生登录、课程、学习任务和通知中心；教师/管理员完整操作矩阵未形成截图或记录 | 使用学生、教师、管理员验收账号补充浏览器端到端记录，覆盖创建、提交、批阅、发布、复核和异常态 | 待手工验收 |
| TST08-RISK-004 | 跨模块真实事件触发链仍需环境级复核 | 干净 H2 HTTP 探针验证 LAB/HWK/GRD/LRN 演示数据闭环；真实发布、评测、成绩发布和通知生成的在线操作链仍需 FAT/UAT 记录 | 在统一验收环境执行“登录 -> 课程 -> 作业/实验 -> 提交/评测 -> 成绩 -> 通知”一次性闭环并保存操作记录 | 待联调确认 |
| TST08-RISK-005 | 本地文件库可能残留旧演示数据 | 2026-06-12 本地 `backend/data/onlinejudge` 启动的 8080 服务曾在 `/api/v1/submissions/950303/evaluation` 返回 500；同一链路在干净 H2 内存库 18080 复跑通过 | 演示或验收前重置本地文件库，或以干净 H2/验收数据库作为验收基准 | 待验收环境复核 |
...
| 最终结论 | 有条件通过 | 当前文档可交给测试负责人整合；需补充 FAT/UAT 浏览器矩阵、专项安全扫描、生产实流量长稳复核和全模块联调记录 |
...
| 前端覆盖     | 有条件通过 | CRS 页面 18 条单测通过，全前端 170 条单测通过；完整 FAT/UAT 浏览器矩阵待补 |
| 数据一致性   | 通过 | CRS 课程、成员、章节、资源、公告及 LRN 近期任务关联在测试中验证 |
| 非功能与安全 | 有条件通过 | 已覆盖基础分页、权限、文件限制和异常映射；本地安全探针和浏览器 smoke 已补，生产扫描与长稳待复核 |
| 最终结论     | 有条件通过 | CRS 自动化与本地联调证据通过，最终验收环境记录待补 |

### 11.3 LRN 验收结论

| 验收维度 | 结论 | 依据 | 残余风险/后续动作 |
| --- | --- | --- | --- |
| 功能完整性 | 有条件通过 | `TC-LN-01 ~ TC-LN-06` 均有可执行用例，后端和前端目标测试通过 | 真实浏览器端到端和跨模块生产事件仍需测试负责人整合验收 |
...
| 测试文档交付 | 有条件通过 | 本文件按 TST-DOC-01 和各模块负责人任务整理 6.3、7.3、8.3、9.3、10、11.3 内容 | 交由 @MontesquieuE 统一整合时需补充最终 FAT/UAT 实测结果 |
...
| 非功能 | 有条件通过 | 可靠性、可追踪性、安全性、可测试性已覆盖；本地性能压测已补充，生产实流量长稳和跨模块真实联调待确认 |
...
| 手工/联调状态 | 自动化、本地 HTTP 安全探针和浏览器 smoke 已补充；生产级安全扫描、生产长稳和 FAT/UAT 手工记录待补 |
...
| 手动测试状态 | 本地 DEV 联调通过；FAT/UAT 验收记录待补 |
...
| 状态口径 | 通过 | 本底稿统一使用“通过 / 有条件通过 / 待手工验收 / 待联调确认 / 待专项测试 / 待验收环境复核 / 待审批” |

### 13.2 跨模块链路人工验收补充用例

| 用例编号 | 链路 | 关联模块 | 验收要点 | 当前状态 |
| --- | --- | --- | --- | --- |
| E2E-INT-001 | 登录 -> 课程 | AUTH/CRS | 教师和学生使用 Bearer 登录态进入课程，课程成员和管理权限正确 | 有条件通过；2026-06-12 干净 H2 HTTP 探针和本地浏览器 smoke 已覆盖学生链路，教师矩阵待 FAT/UAT 复验 |
| E2E-INT-002 | 课程 -> 作业 -> 提交/评测 | AUTH/CRS/HWK/LRN | 教师发布作业，学生提交，评测/批阅完成，LRN 生成任务和通知 | 有条件通过；HTTP 探针验证作业任务、评测结果和通知数据，真实提交/批阅操作待 FAT/UAT 复验 |
| E2E-INT-003 | 课程 -> 实验 -> 提交/评测 | AUTH/CRS/LAB/LRN | 教师发布实验，学生提交代码/报告，评测和评分完成，LRN 生成任务和通知 | 有条件通过；HTTP 探针验证实验任务和 LAB 通知，真实提交/评分操作待 FAT/UAT 复验 |
| E2E-INT-004 | 作业/实验 -> 成绩 | LAB/HWK/GRD | LAB/HWK 发布成绩后，GRD 同步来源成绩并计算总评 | 有条件通过；HTTP 探针验证 LAB/HWK 来源链和总评 89.6，真实同步按钮待 FAT/UAT 复验 |
| E2E-INT-005 | 成绩 -> 通知 | GRD/LRN | 教师发布成绩或处理异议后，学生在通知中心看到对应通知并可跳转 | 有条件通过；HTTP 探针和浏览器通知页验证 GRD 通知可见，真实发布/复核操作待 FAT/UAT 复验 |
| E2E-INT-006 | 全链路 | AUTH/CRS/LAB/HWK/GRD/LRN | 登录 -> 课程 -> 作业/实验 -> 提交/评测 -> 成绩 -> 通知一次性闭环 | 有条件通过；2026-06-12 本地 clean H2 HTTP 闭环和浏览器 smoke 通过，最终验收环境待确认 |
