...
本文件用于记录 LAB 实训实验模块在当前版本下的测试依据、测试环境、测试数据、测试用例、执行结果、手工验收清单、缺陷风险和验收结论。覆盖范围对齐 `FR-LAB-01 ~ FR-LAB-08`、`NFR-LAB-01 ~ NFR-LAB-05`、`UI-LAB-01 ~ UI-LAB-08`、`API-LAB-01 ~ API-LAB-18`、`DB-LAB-01 ~ DB-LAB-07`、`TC-LAB-01 ~ TC-LAB-33`、`TC-LAB-N01 ~ TC-LAB-N05`，以及为本 issue 统一整合而补充的 `IC-LAB-01 ~ IC-LAB-05`、`IC-LAB-N01 ~ IC-LAB-N05` 跨模块集成追踪编号。
...
| --- | --- |
| 操作系统 | Windows |
| 后端运行环境 | Java 24，Spring Boot 3.4.5，Maven 3.9.9，JUnit 5，MockMvc，H2 MySQL mode |
| 前端运行环境 | Node.js 24.15.0，Vue 3.5，Vite 6.3，Vitest 3.1，jsdom |
...
| --- | --- | --- | --- | --- |
| LAB-LOG-001 | 2026-06-11 21:56 | `LabExperimentControllerTest` | 实验创建、发布、截止、成绩发布、统计、权限与异常矩阵 | 8 条通过 |
| LAB-LOG-002 | 2026-06-11 21:56 | `LabSubmissionControllerTest` | 提交、历史、评测、报告、评分、结果展示、GRD 来源成绩与权限边界 | 29 条通过 |
| LAB-LOG-003 | 2026-06-11 21:56 | `LabExperimentMigrationTest` | 迁移脚本、报告表、评分表、评分变更表与软删除约束 | 7 条通过 |
| LAB-LOG-004 | 2026-06-11 21:56 | `LabExperimentTransactionTest` | 实验与测试用例创建/更新的事务回滚 | 2 条通过 |
| LAB-LOG-005 | 2026-06-11 21:56 | `LabEvaluationServiceTest` | 评测器异常时状态落库与失败结果留痕 | 1 条通过 |
| LAB-LOG-006 | 2026-06-11 21:56 | Maven 汇总 | `Tests run: 47, Failures: 0, Errors: 0, Skipped: 0` | 构建成功 |

### 8.2 前端 LAB 执行日志

| 日志编号 | 时间 | 命令/测试文件 | 执行内容 | 结果 |
| --- | --- | --- | --- | --- |
| LAB-LOG-007 | 2026-06-11 21:56 | `tests/unit/api/labs.spec.ts` | LAB API wrapper：报告下载、报告评分、统计接口 | 3 条通过 |
| LAB-LOG-008 | 2026-06-11 21:56 | `tests/unit/lab/LabTeacherView.spec.ts` | 教师端实验管理、统计、提交筛选、报告和评分交互 | 11 条通过 |
| LAB-LOG-009 | 2026-06-11 21:56 | `tests/unit/lab/LabStudentView.spec.ts` | 学生端实验详情、提交、评测结果、报告上传下载、成绩展示 | 12 条通过 |
| LAB-LOG-010 | 2026-06-11 21:56 | `tests/unit/lab/LabSubmissionHistoryView.spec.ts` | 提交历史、空态、失败提示 | 3 条通过 |
| LAB-LOG-011 | 2026-06-11 21:56 | Vitest 汇总 | `Test Files 4 passed (4)`、`Tests 29 passed (29)` | 构建成功 |
...
| 风险编号 | 风险说明 | 影响范围 | 建议处理 |
| --- | --- | --- | --- |
| R-LAB-001 | issue #156 要求追踪 `IC-LAB-*` 编号，但正式设计文档未定义该编号体系。 | 集成追踪一致性 | 本文已按 issue 要求补充文档级集成编号，并在 4.3 节标明其来源和对应关系。 |
