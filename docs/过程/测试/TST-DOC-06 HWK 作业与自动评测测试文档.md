...
| --- | --- | --- | --- |
| FR-HWK-01 | 作业创建与发布 | 教师/助教创建草稿、编辑、原子逻辑删除 DRAFT、保存题目/测试用例、发布、关闭、发布通知；删除只改父表，普通更新不得复活 | v1 实现 PASS；#338 v2 目标为 Homework + outbox 本地成功即 PUBLISHED，Learning/broker 不回滚，只有本地失败为 `503/HWK_5003`/DRAFT；运行时验收待 #337/服务拆分 Issue |
