...
| 测试结论 | #264 的场景、三层图与多数自动化路径已通过，GRD 成绩消费已按 `gradeItemId + studentId + sourceId + rawScore` 精确复测为 PASS；但完整闭环当前仍为 FAIL：SRS 4.10 与通知投递失败运行时语义不一致，独立缺陷 #281 跟踪，实际完成日期暂不填写 |
...
| --- | --- | --- | --- |
| FR-HWK-01 | 作业创建与发布 | 教师/助教创建草稿、编辑、原子逻辑删除 DRAFT、保存题目/测试用例、发布、关闭、发布通知；删除只改父表，普通更新不得复活 | 主流程与草稿删除通过；通知投递失败语义与 SRS 4.10 不一致，FAIL，见 #281 |
...
| NFR-HWK-01 | 可靠性 | 提交、评测、批阅、通知失败和分数记录不丢失；草稿删除原子化且普通更新不能复活 | 草稿与提交可靠性通过；通知失败缺少与 SRS 一致的可追踪结果，FAIL，见 #281 |
