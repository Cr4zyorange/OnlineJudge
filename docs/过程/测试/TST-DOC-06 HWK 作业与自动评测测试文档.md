...
| 测试结论 | #296 已补齐 CODE 后台 Worker 与持久恢复：提交创建 PENDING 任务，事务提交后独立消费并进入终态；定时扫描重投首次派发遗漏任务，启动时重置旧进程遗留 RUNNING；API-HWK-11 保持纯读取，异常落 SYSTEM_ERROR 且提交保留。FR-HWK-04、NFR-HWK-01 与 TC-HWK-10/11 本地自动化判定 PASS；真实 Docker 专项仍按部署环境复核 |
...
| FR-HWK-04 | 自动评测 | 客观题自动评分、代码题 IO 评测、失败状态保留、评测结果查询、重评 | PASS：客观题同步评分；CODE 提交后由后台 Worker 从 PENDING 推进到终态，定时扫描重投遗漏任务、启动恢复旧 RUNNING，API-HWK-11 仅读，异常保留提交 |
...
| NFR-HWK-01 | 可靠性 | 提交、评测、批阅、通知失败和分数记录不丢失；草稿删除原子化且普通更新不能复活 | PASS；通知失败不留下已发布作业或部分通知；CODE PENDING 在首次派发遗漏时由定时扫描重投，旧进程 RUNNING 在启动时重新入队，受控错误与事务状态可追踪 |
...
| TC-HWK-10 | FR-HWK-04 | UI-HWK-05、07；API-HWK-07、10、11；DB-HWK-03、05 | 代码题配置 IO 用例和语言白名单；提交已持久化但首次 after-commit 派发未发生 | 学生提交代码；不调用 API-HWK-11，仅轮询提交详情；恢复器扫描持久 PENDING | 独立 Worker 将 PENDING 推进到终态并返回通过用例数和分数 | `codeHomeworkSubmissionEvaluatesIoCasesAndTeacherCanReevaluate` 不读取 API-HWK-11 即等待到 ACCEPTED；`recoveryEvaluatesPersistedCodeSubmissionWhenInitialAfterCommitDispatchWasMissed` 证明恢复器可推进遗漏任务；E2E 仅轮询 API-HWK-10 并断言 100 分 | 通过 |
| TC-HWK-11 | FR-HWK-04；NFR-HWK-01 | API-HWK-07、10、11；DB-HWK-04、05 | 代码提交包含编译错误、评测器异常或旧进程遗留 RUNNING | 学生提交代码；不调用 API-HWK-11，仅轮询提交详情；启动恢复后重新投递旧 RUNNING | Worker 记录 COMPILE_ERROR/SYSTEM_ERROR，提交记录不丢失；旧进程任务不永久滞留 RUNNING | E2E 编译错误进入 COMPILE_ERROR；`codeHomeworkWorkerFailurePreservesSubmissionAndRecordsSystemError` 证明异常落 SYSTEM_ERROR 且提交保留；`recoveryRequeuesRunningCodeSubmissionLeftByPreviousProcess` 证明启动恢复后进入终态 | 通过 |
...
| TC-HWK-N01 | NFR-HWK-01 | API-HWK-03、07、11、13；DB-HWK-05 | 模拟通知投递失败、评测失败、重复提交冲突、首次派发遗漏和进程重启 | 执行发布、提交、查询和批阅；扫描 PENDING 并启动恢复 RUNNING | 主数据保持一致，错误以受控响应返回，持久评测任务不会因短暂派发异常或进程重启永久滞留 | `publishRollsBackHomeworkWhenRequiredNotificationDeliveryFails` 断言 `503/HWK_5003`、`DRAFT` 与无通知残留；两个 `HomeworkEvaluationRecovery` 自动化用例覆盖遗漏派发与重启恢复 | 通过 |
...
| HWK-LOG-033 | 2026-08-27 | `HomeworkEvaluationRecoveryTest`、`HomeworkControllerTest`、`mvn -q test` | 持久 PENDING 在首次派发遗漏后被扫描重投；旧进程遗留 RUNNING 在启动时重置并重新执行；无 HWK 表的其他模块测试上下文默认关闭恢复器 | 后端相邻 105/105、全量 394（387 passed、7 skipped），0 failures、0 errors，PASS |
...
| R-HWK-002 | #296 已消除读请求触发评测和任务派发遗漏/重启永久滞留缺陷；剩余风险为真实 Docker 多语言、资源限制与高并发专项未在本机执行 | FR-HWK-04、NFR-HWK-01、NFR-HWK-02、NFR-HWK-05 | 后台 Worker、原子认领、PENDING 定时恢复、旧 RUNNING 启动恢复、纯读取与 SYSTEM_ERROR 兜底已自动化通过；部署环境继续执行真实 Docker 专项，不回退为 API-HWK-11 同步评测 |
...
| 验收项 | 结论 | 说明 |
| --- | --- | --- |
| 功能覆盖 | 通过 | 固定五档/attention、草稿逻辑删除等既有证据保留；#296 补齐 CODE 后台 Worker 与持久恢复，FR-HWK-04、NFR-HWK-01 与 TC-HWK-10/11 通过 |
| 接口覆盖 | 通过 | API-HWK-07 创建 PENDING 任务并在提交后异步消费；PENDING 定时恢复与旧 RUNNING 启动恢复均为内部实现；API-HWK-10/11 只读状态，不新增或修改公共请求/响应契约 |
...
| 非功能 | 有条件通过 | 分页、稳定排序、SQL 聚合、索引和后台任务可重复认领/恢复已有证据；生产压测、真实 MySQL EXPLAIN 和真实 Docker 专项仍待部署环境执行 |
| 最终结论 | #296 PASS | 后台 Worker、PENDING 定时恢复、旧 RUNNING 启动恢复、API-HWK-11 纯读取、异常终态和不读取 API-HWK-11 的验收契约已补齐；#264 已由 PR #276 完成文档与测试交付，#296 合并后独立关闭 FR-HWK-04 产品缺陷 |
...
| #296 自动化 | 后端相邻 105/105、全量 394（387 passed、7 skipped）；首次派发遗漏与旧进程 RUNNING 恢复均进入终态；前端 typecheck、189 modules build、Playwright 2 条用例加载、文档/SVG 语义契约通过；Compose runner 因 Docker daemon 不可用待复测 |
