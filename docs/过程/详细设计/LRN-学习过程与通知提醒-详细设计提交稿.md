...
应用内直接调用由 `PersistentNotificationEventPublisher` 统一登记事务同步回调：存在活动来源事务时，在 `afterCommit` 阶段以独立新事务调用 `NotificationService`；来源事务回滚时不执行回调。没有活动事务时直接开启独立事务落库。通知落库失败仅记录告警，不回滚已经提交的来源业务；持久重试/outbox 不在本次实现范围。
...
| 通知事件持久化失败 | after-commit 独立事务回滚并记录告警，不反向回滚来源业务；当前未实现持久重试/outbox | 30002  | 来源业务响应不反向改写 |
