...
#### 6.3.6 t_grade_analysis_source_version 统计来源版本表
...
5. 成绩发布：发布前检查规则和未评分状态；发布后学生可查看；相同范围重复发布返回同一 `publishId`，不产生重复通知或重复发布记录；通知持久化失败只记告警，当前 `notificationStatus` 仍为 `SENT`。
