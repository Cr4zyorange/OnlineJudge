...
| DB-GRD-08 | t_grade_analysis_snapshot | 统计分析快照表 | id, course_id, target_type, grade_item_id, source_data_time, source_fingerprint, average_score, max_score, min_score, pass_rate, completion_rate, distribution_json, calculated_at | 保存统计分析结果、来源版本指纹及来源时间点 |
...
| source_fingerprint | varchar(64) | 是 | 来源版本 SHA-256；历史快照可为空，新增快照必须写入 |
...
7. 统计分析快照必须记录 source_data_time 和 source_fingerprint；指纹覆盖统计目标、当前有效学生集合以及对应成绩或总评的状态、分数和更新时间。指纹一致时复用最新快照，指纹变化时生成新快照并推进来源时间点。
...
6. 教学分析保存 GradeAnalysisSnapshot；同一课程、统计目标和成绩项的来源指纹未变化时复用最新快照，不追加重复行。成绩、总评或当前有效学生集合变化后重新计算并保存新快照。
