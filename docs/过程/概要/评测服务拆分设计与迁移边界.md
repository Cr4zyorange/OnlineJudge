...
>
> 设计状态：历史迁移输入，尚未实施服务拆分；当前五服务目标以 #305 的
> [D6-D7-五服务架构冻结-305.md](../../开发/D6-D7-五服务架构冻结-305.md) 为唯一正本。
...
本节保留 #279 当时提出的四服务近似模型，用于审计迁移差异；它已被 #305 的五服务冻结替代，其中“学习与成绩服务”拆为 Grade 和 Learning。下表不是新服务实现的正本：
...
| Assessment -> Learning（v2 作业发布） | `assessment.homework.published.v2` transactional outbox | Homework 发布时在同一**本地**事务写入 `PUBLISHED`/`publishedAt` 与 outbox；事件包含 homeworkId、courseId、title（1–100 字符）、RFC3339 deadline、receiverScope=COURSE_ACTIVE_STUDENTS、publishedAt，禁止 roster | 本地 Homework/outbox 事务失败才返回 `503/HWK_5003` 并保持 `DRAFT`；Learning、broker 或网络不可用不得回滚。Learning 以成员投影按 receiverScope 幂等创建任务/通知 |
| 评测服务 -> 学习与成绩服务（尽力而为通知） | `NotificationEventPublisher.publish(NotificationEvent)` | `LAB_EXPERIMENT_PUBLISHED`、`LAB_SUBMISSION_SCORED`、`EXPERIMENT_SCORE_PUBLISHED`、`HOMEWORK_SCORE_PUBLISHED`；后续若补齐 HWK 评测完成通知，按既有命名约定新增 `HOMEWORK_EVALUATION_FINISHED` | 使用既有 `idempotencyKey` 去重；有活动来源事务时仅在 after-commit 投递，调度或落库失败记录可诊断错误，不回滚已提交的 LAB/HWK 业务事务 |
...
服务化实施时，上述 Java 接口分别替换为认证后的内部 HTTP 客户端或事件投递适配器，保留相同语义与字段。身份服务仍提供认证主体；评测服务不得信任浏览器请求体中伪造的 `userId`、角色或课程权限。Assessment API/Worker 的 durable task、lease、heartbeat、generation 和 fenced final write 以 #305 正本为准，本历史文档不另行发明第二套 worker 契约。
...
4. 将 `CoursePermissionClient`、`NotificationEventPublisher`、`SourceGradeClient` 换为跨服务实现后，先通过 v2 Homework+outbox 本地事务、Learning 不可用不回滚、事件自足字段/无 roster 的契约测试和全量业务 E2E，再停止单体内直连实现。
...
| 通知一致性 | 当前单体 `HomeworkService.publish`/`publishRequired` 是 v1 历史基线 | 跨服务契约测试必须验证：Homework + outbox 本地成功即 PUBLISHED；Learning/broker 不可用不回滚；仅本地事务失败返回 `503/HWK_5003`/DRAFT；事件具备 title/deadline/receiverScope 且无 roster；普通 v1 `publish` 的历史行为不构成 v2 生产路径 |
...
- #338 已将 API-HWK-03 的显式 v2 语义变更同步至三份 `docs/最终提交/` 主文档及 HWK/LRN 过程文档；当前单体 `publishRequired` 证据仅作为 v1 历史基线，不可用来接受 v2 服务化实现。
