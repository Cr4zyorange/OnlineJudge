...
| 评测服务 -> 学习与成绩服务（必达发布） | `NotificationEventPublisher.publishRequired(NotificationEvent)` | HWK 发布时同步发送 `HOMEWORK_PUBLISHED`；事件名称、载荷和既有 `idempotencyKey` 不变 | 必达投递失败向上抛出并映射为 `503/HWK_5003`，发布事务整体回滚，作业保持 `DRAFT`，不得生成部分通知 |
...
4. 将 `CoursePermissionClient`、`NotificationEventPublisher`、`SourceGradeClient` 换为跨服务实现后，先通过通知必达/尽力而为两条分支的契约测试和全量业务 E2E，再停止单体内直连实现。
...
| 通知一致性 | 当前 `HomeworkService.publish` 与 `NotificationEventPublisher` 已定义必达/尽力而为语义 | 跨服务契约测试必须分别验证：`HOMEWORK_PUBLISHED` 走 `publishRequired`，通知失败返回 `503/HWK_5003`、作业回滚为 `DRAFT` 且不生成部分通知；普通 `publish` 在 after-commit 调度或落库失败时仅记录告警，不回滚已提交业务 |
