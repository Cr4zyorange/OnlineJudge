...
| 独立场景 | UC-GR-02 配置成绩项与规则 | 保存来源类型、满分、权重；LAB/HWK 使用正整数来源编号，OTHER_COURSE_ITEM 可为空 | 来源类型非法、LAB/HWK 编号为空或非正数、权重非法、重复名称、已发布规则限制；LAB/HWK 来源可用性在同步阶段确认 | 图 4-20 | 8.1 | 7.1 |
