...
| 公共子流程 | 调整留痕与 LRN 通知 | 写变更日志；响应前同步 best-effort 调用 LRN 通知持久化；publisher 挂起外层 GRD 事务，LRN 使用独立事务 | 通知失败只回滚 LRN 事务并记录告警，不将 GRD 事务标记为 rollback-only，不回滚成绩/发布/复核结果；复核响应不含通知结果，发布响应仍返回持久化前写入的 `notificationStatus=SENT` | 图 4-15、图 4-40 | 8.3、8.6 | 7.3、7.5、图 3-6-12 |
