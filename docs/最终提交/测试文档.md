...
| 测试结论 | 通过；后端/前端自动化、本地真实服务闭环探针、本地安全探针、Docker Python 基础专项和本地 DEV 浏览器手工矩阵均已通过，覆盖最终提交要求的小并发演示环境和核心业务闭环 |
...
| V1.1 | 2026-06-12 | 项目负责人 | 依据 issue #160 完成最终审核，统一最终提交口径，明确环境级复核项不阻塞本次最终提交 |

### 1.2 审批记录

| 角色 | 姓名/负责人 | 审批意见 | 日期 |
| --- | --- | --- | --- |
| 项目负责人 | @Lucio-ball | 通过，同意作为最终提交测试文档 | 2026-06-12 |
| 测试负责人 | @MontesquieuE | 通过，测试范围、执行记录和结论口径已完成整合 | 2026-06-12 |
...
| 多用户并发选课/审批 | 自动化覆盖重复加入、满员、待审批、拒绝后重申等单请求链路，本地性能批次已覆盖小并发演示规模 | 生产实流量长稳作为部署复核项 |
...
| 环境项 | 内容 |
| --- | --- |
| 操作系统 | Windows / macOS 本地开发环境；本轮最终审核复核在 macOS 环境完成 |
...
| MAN-AUTH-005 | AUTH | 权限边界页面 | 学生访问教师入口和管理员后台；教师访问管理员后台；管理员访问审计日志 | 越权页面显示 403；接口返回 403；管理员可查看日志 | 通过；2026-06-12 本地安全探针验证匿名 401、伪造 Header 401、伪造 Bearer 401、学生访问管理接口 403 |
...
| MAN-AUTH-008 | AUTH | 专项安全复核 | 使用自动化安全用例和本地安全探针扫描认证、管理和资料接口；OWASP/ZAP 或等效工具作为上线前复核 | 无高危认证绕过、敏感信息泄露、明文密码或令牌泄露 | 通过；本次最终提交以自动化安全用例和本地安全探针为验收依据，生产级扫描不作为课程项目阻塞项 |
| MAN-AUTH-009 | AUTH | 性能和并发 | 准备大量用户、会话和审计日志，压测登录、`/me`、权限校验、审计分页 | 常规认证和权限接口满足 3 秒响应要求，分页稳定 | 通过；本地压测覆盖小并发演示环境，生产实流量长稳作为部署复核项 |
...
| LRN-MAN-010 | TC-LN-N02、TC-LN-N03 | 通知触达时延和可靠性 | 测试环境支持通知轮询或推送 | 触发任务/成绩/公告通知并计时 | 通知列表和未读数在设计阈值内刷新，断线恢复后不丢通知 | 通过；2026-06-12 本地 DEV 验证列表刷新、已读、删除和业务跳转，正式部署环境可按同一用例补充计时留痕 |
...
| MAN-LAB-008 | LAB | 真实 Docker 沙箱专项测试 | 提交 Python AC/编译错误/运行错误/超时/内存超限样本，执行并发容器样本 | 状态、日志、资源限制和超时控制符合设计 | 通过；2026-06-12 `OJ_DOCKER_SANDBOX_TEST=true mvn -Dtest=DockerSandboxExecutorTest test` 复跑 2 条通过，真实容器专项覆盖 Python 基础自动评测能力 |
...
| MAN-HWK-007 | HWK/LAB | 真实代码评测沙箱 | 使用真实 Docker 沙箱提交 Python 代码，包含 AC、编译错误、运行错误、超时和内存限制样本 | 状态、日志、资源限制和隐藏用例显示策略正确 | 通过；真实 Docker Python 执行器测试通过，HWK 真实提交/批阅链路已在 `950302`/`950304` 验证 |
...
| --- | --- | --- | --- | --- |
| TST-FINAL-RISK-001 | 生产级安全扫描未作为本次最终提交依据 | 后端、前端自动化和 2026-06-12 本地安全探针覆盖匿名、伪造 Header、伪造 Bearer、学生越权等核心边界 | 上线前可补充 OWASP ZAP 或等效工具的扫描目标、时间、工具版本、报告编号和处置结论 | 非阻塞部署复核项 |
| TST-FINAL-RISK-002 | 生产实流量长稳和真实并发审批未纳入课程项目验收范围 | 本地性能批次覆盖 154 条后端样本，课程/资源 105 条基础规模样本通过，满足需求文档定义的小并发演示环境 | 部署到统一验收环境后，可按同一主流程补充连续运行日志 | 非阻塞部署复核项 |
| TST-FINAL-RISK-003 | FAT/UAT 正式账号截图和记录编号未纳入本文档 | 2026-06-12 已在本地 DEV 浏览器验证学生、教师、管理员三类角色矩阵，覆盖注册、资料、管理员、学习、通知、实验、作业、成绩和复核操作 | 课程答辩或部署验收时可补充截图编号，不改变本文测试结论 | 非阻塞留痕项 |
| TST-FINAL-RISK-004 | 跨模块事件链需在部署环境保持相同配置 | 本地 DEV 已执行“登录 -> 课程 -> 作业/实验 -> 提交/评测 -> 成绩 -> 通知/复核”闭环；成绩同步返回 `syncedCount=2`、`affectedStudentCount=1` | 部署环境复跑一次闭环并保存操作记录 | 非阻塞部署复核项 |
| TST-FINAL-RISK-005 | 旧本地文件库可能残留演示数据 | 2026-06-12 本地 `backend/data/onlinejudge` 启动的 8080 服务曾在 `/api/v1/submissions/950303/evaluation` 返回 500；后续手工矩阵使用干净 H2 内存库 `tst08_manual` 在 8080 复跑通过 | 演示或验收前重置本地文件库，或以干净 H2/验收数据库作为验收基准 | 已规避 |
...
| --- | --- | --- | --- |
| 功能完整性 | 通过 | `TC-LN-01 ~ TC-LN-06` 均有可执行用例，后端和前端目标测试通过，2026-06-12 本地浏览器补充断点、通知、提醒真实操作 | 部署环境复核不影响最终提交结论 |
...
| 非功能 | 通过 | 自动化覆盖分页、size 上限、幂等、离线队列、状态日志和失败重试；2026-06-12 本地压测批次通过 | 正式通知触达计时可在部署环境补充 |
...
| 非功能 | 通过 | 可靠性、可追踪性、安全性、可测试性已有自动化证据；本地性能压测和真实 Docker Python 基础专项已补充 |
...
| 非功能 | 通过 | 可靠性、可追踪性、安全性、可测试性已覆盖；本地压测和真实 Docker Python 基础专项已补充 |
...
| 非功能 | 通过 | 可靠性、可追踪性、安全性、可测试性已覆盖；本地性能压测和本地跨模块联调已补充，生产实流量长稳作为部署复核项 |
...
| 手工/联调状态 | 自动化、本地 HTTP 安全探针、本地浏览器 smoke 和 2026-06-12 本地 DEV 手工矩阵已补充；生产级安全扫描、生产长稳和 FAT/UAT 留痕为非阻塞部署复核项 |
...
| 手动测试状态 | 本地 DEV 联调和浏览器矩阵通过；FAT/UAT 正式留痕为非阻塞部署复核项 |
...
| 手工/联调状态 | 2026-06-12 本地 DEV 已补真实浏览器作业创建、发布、提交、批阅、发布成绩和 GRD 同步记录；生产/FAT/UAT 留痕为非阻塞部署复核项 |
...
| 手工/联调状态 | 2026-06-12 本地 DEV 已补真实浏览器成绩表、来源成绩同步、通知跳转和成绩复核记录；生产规模性能和 FAT/UAT 留痕为非阻塞部署复核项 |
...
| 状态口径 | 通过 | 本最终版统一使用“通过 / 非阻塞部署复核项 / 非阻塞留痕项 / 已规避”，避免把部署留痕误写为最终提交阻塞项 |
...
| TST08-LOCALSTATE-001 | 本地文件库后端 `http://127.0.0.1:8080` | 旧文件库演示数据一致性抽查 | `/api/v1/submissions/950303/evaluation` 曾返回 500；同链路切换干净 H2 内存库后通过 | 已规避；最终提交以干净 H2 或验收数据库为基准 |
...
| TST08-MANUAL-010 | 保留的环境级事项 | 生产级安全扫描、生产实流量长稳、FAT/UAT 正式账号截图或测试记录编号不属于本地 DEV 可证明范围；当前真实 Docker 专项覆盖 Python 基础自动评测能力 | 通过；上述事项为非阻塞部署复核或留痕项 |
