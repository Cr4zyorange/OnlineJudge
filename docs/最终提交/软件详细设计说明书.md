...
| NFR-CR-02 | 可靠性 | DSD-CRS-NFR-02 | UI-CRS-01 ~ UI-CRS-07 | CRS-API-01 ~ CRS-API-22 | DB-CRS-01 ~ DB-CRS-05 | TC-CR-N02 | 课程数据一致性、成员关系正确性 |
| NFR-CR-03 | 可用性 | DSD-CRS-NFR-03 | UI-CRS-01 ~ UI-CRS-07 | CRS-API-01 ~ CRS-API-22 | - | TC-CR-N03 | 页面操作流畅、错误提示清晰 |
| NFR-CR-04 | 性能 | DSD-CRS-NFR-04 | UI-CRS-01、UI-CRS-04 | CRS-API-05、CRS-API-11 | DB-CRS-01 ~ DB-CRS-05 | TC-CR-N04 | 课程列表分页、资源上传大小限制 |
...
6. 不实现第三方邮件、短信推送和移动端推送，首版以站内通知为主。
...
- **评测方案**：首版采用 IO 比对评测方案（标准输入输出匹配），评测服务通过统一接口抽象，保证后续可以与作业评测能力复用或替换实现。
...
| LRN | 学习任务聚合、通知生成、通知读取、提醒规则 | 被 LAB、HWK、GRD 触发通知事件 | 首版以站内通知为主，支持定时提醒规则 |
...
| UI-LAB-02 | 实验详情页（学生端） | LAB | 学生 | 查看实验要求、提交实验、查看结果 | 查看实验描述与附件、在线编写代码或上传文件、提交、查看评测结果与教师评分 |
| UI-LAB-05 | 提交历史页 | LAB | 学生 | 查看本人某次实验的全部提交记录 | 查看提交时间、提交版本、评测状态、评测得分、查看提交详情 |
| UI-LAB-07 | 实验结果页 | LAB | 学生 | 查看实验最终成绩和反馈 | 查看最终成绩、教师评语、评测通过率、各测试用例结果 |
...
| UI-LAB-06 | 评分页 | LAB | 教师 | 对学生提交进行评分 | 查看学生提交代码、评测结果、填写评分和评语、保存评分 |
| UI-LAB-08 | 实验统计页 | LAB | 教师 | 查看实验维度统计数据 | 查看提交率、平均分、分数分布、提交时间分布 |
| UI-HWK-01 | 作业中心页 | HWK | 学生、教师、助教 | 汇总展示用户可见的作业列表 | 学生查看待完成/已提交/已截止作业；教师查看草稿/已发布/已关闭作业；按课程、状态、关键词筛选 |
...
系统接口采用 RESTful 风格，统一以 `/api` 或 `/api/v1` 作为接口前缀。请求和响应数据主要采用 JSON 格式，文件上传接口使用 `multipart/form-data`。所有业务接口需明确请求方法、路径、权限要求、请求参数、响应字段、异常码和对应需求编号。
...
| --- | --- | --- | --- | --- | --- | --- |
| API-AUTH-01 | AUTH | 用户登录 | POST | ``/api/auth/login`` | 无需登录 | FR-UA-01 |
| API-AUTH-02 | AUTH | 用户注册 | POST | ``/api/auth/register`` | 无需登录或管理员 | FR-UA-01 |
| API-AUTH-03 | AUTH | 用户退出登录 | POST | ``/api/auth/logout`` | 当前用户已登录 | FR-UA-01 |
| API-AUTH-04 | AUTH | 获取当前用户信息 | GET | ``/api/auth/me`` | 当前用户已登录 | FR-UA-03 |
| API-AUTH-05 | AUTH | 查看个人资料 | GET | ``/api/users/me`` | 当前用户已登录 | FR-UA-04 |
| API-AUTH-06 | AUTH | 修改个人资料 | PUT | ``/api/users/me`` | 当前用户已登录 | FR-UA-04 |
| API-AUTH-07 | AUTH | 修改密码 | PUT | ``/api/users/me/password`` | 当前用户已登录 | FR-UA-04 |
| API-AUTH-08 | AUTH | 查询用户列表 | GET | ``/api/admin/users`` | 管理员 | FR-UA-02 |
| API-AUTH-09 | AUTH | 管理员创建用户 | POST | ``/api/admin/users`` | 管理员 | FR-UA-02 |
| API-AUTH-10 | AUTH | 修改账号状态 | PUT | ``/api/admin/users/{userId}/status`` | 管理员 | FR-UA-02、FR-UA-05 |
| API-AUTH-11 | AUTH | 查询角色列表 | GET | ``/api/admin/roles`` | 管理员 | FR-UA-02 |
| API-AUTH-12 | AUTH | 创建或修改角色 | POST/PUT | ``/api/admin/roles`` | 管理员 | FR-UA-02 |
| API-AUTH-13 | AUTH | 查询权限点列表 | GET | ``/api/admin/permissions`` | 管理员 | FR-UA-02 |
| API-AUTH-14 | AUTH | 调整角色权限 | PUT | ``/api/admin/roles/{roleId}/permissions`` | 管理员 | FR-UA-02、FR-UA-06 |
| API-AUTH-15 | AUTH | 调整用户角色 | PUT | ``/api/admin/users/{userId}/roles`` | 管理员 | FR-UA-02、FR-UA-06 |
| API-AUTH-16 | AUTH | 通用权限校验 | POST | ``/api/auth/check-permission`` | 当前用户已登录 | FR-UA-03 |
| API-AUTH-17 | AUTH | 查询审计日志 | GET | ``/api/admin/audit-logs`` | 管理员 | FR-UA-06 |
| CRS-API-01 | CRS | 创建课程 | POST | `/api/courses` | 已登录教师 | FR-CR-01 |
| CRS-API-02 | CRS | 获取课程详情 | GET | `/api/courses/{courseId}` | 课程成员 | FR-CR-01 |
| CRS-API-03 | CRS | 更新课程 | PUT | `/api/courses/{courseId}` | 课程教师 | FR-CR-01 |
| CRS-API-04 | CRS | 删除课程 | DELETE | `/api/courses/{courseId}` | 课程教师 | FR-CR-01 |
| CRS-API-05 | CRS | 获取课程列表 | GET | `/api/courses` | 已登录用户 | FR-CR-01 |
| CRS-API-06 | CRS | 创建章节 | POST | `/api/courses/{courseId}/chapters` | 课程教师 | FR-CR-03 |
| CRS-API-07 | CRS | 获取章节树 | GET | `/api/courses/{courseId}/chapters` | 课程成员 | FR-CR-03 |
| CRS-API-08 | CRS | 修改章节 | PUT | `/api/courses/{courseId}/chapters/{chapterId}` | 课程教师 | FR-CR-03 |
| CRS-API-09 | CRS | 删除章节 | DELETE | `/api/courses/{courseId}/chapters/{chapterId}` | 课程教师 | FR-CR-03 |
| CRS-API-10 | CRS | 上传教学资源 | POST | `/api/courses/{courseId}/resources` | 课程教师 | FR-CR-04 |
| CRS-API-11 | CRS | 获取资源列表 | GET | `/api/courses/{courseId}/resources` | 课程成员 | FR-CR-04 |
| CRS-API-12 | CRS | 更新资源 | PUT | `/api/courses/{courseId}/resources/{resourceId}` | 课程教师 | FR-CR-04 |
| CRS-API-13 | CRS | 删除资源 | DELETE | `/api/courses/{courseId}/resources/{resourceId}` | 课程教师 | FR-CR-04 |
| CRS-API-14 | CRS | 学生选课 | POST | `/api/courses/{courseId}/join` | 已登录用户 | FR-CR-02 |
| CRS-API-15 | CRS | 获取课程成员列表 | GET | `/api/courses/{courseId}/members` | 课程成员 | FR-CR-05 |
| CRS-API-16 | CRS | 管理成员角色 | PUT | `/api/courses/{courseId}/members/{userId}` | 课程教师 | FR-CR-05 |
| CRS-API-17 | CRS | 移除课程成员 | DELETE | `/api/courses/{courseId}/members/{userId}` | 课程教师 | FR-CR-05 |
| CRS-API-18 | CRS | 获取选课学生名单 | GET | `/api/courses/{courseId}/students` | 课程教师、HWK/LAB/GRD模块 | FR-CR-05 |
| CRS-API-19 | CRS | 校验课程权限 | GET | `/api/courses/{courseId}/permissions/{userId}` | HWK/LAB/GRD模块 | FR-CR-05 |
| CRS-API-20 | CRS | 发布公告 | POST | `/api/courses/{courseId}/announcements` | 课程教师 | FR-CR-06 |
| CRS-API-21 | CRS | 获取公告列表 | GET | `/api/courses/{courseId}/announcements` | 课程成员 | FR-CR-06 |
| CRS-API-22 | CRS | 课程首页摘要 | GET | `/api/courses/{courseId}/home-summary` | 课程成员 | FR-CR-06 |
...
| API-LAB-01 | LAB | 创建实验 | POST | `/api/courses/{courseId}/labs` | 教师（课程管理权限） | FR-LAB-01 |
| API-LAB-02 | LAB | 获取实验列表 | GET | `/api/courses/{courseId}/labs` | 学生、教师（课程成员） | FR-LAB-01 |
| API-LAB-03 | LAB | 获取实验详情 | GET | `/api/labs/{labId}` | 学生、教师（课程成员） | FR-LAB-01 |
| API-LAB-04 | LAB | 更新实验 | PUT | `/api/labs/{labId}` | 教师（课程管理权限） | FR-LAB-01 |
| API-LAB-05 | LAB | 删除实验 | DELETE | `/api/labs/{labId}` | 教师（课程管理权限） | FR-LAB-01 |
| API-LAB-06 | LAB | 发布实验 | POST | `/api/labs/{labId}/publish` | 教师（课程管理权限） | FR-LAB-01 |
| API-LAB-07 | LAB | 截止实验 | POST | `/api/labs/{labId}/close` | 教师（课程管理权限） | FR-LAB-01 |
| API-LAB-08 | LAB | 学生提交实验 | POST | `/api/labs/{labId}/submissions` | 学生（课程成员） | FR-LAB-02 |
| API-LAB-09 | LAB | 获取提交列表 | GET | `/api/labs/{labId}/submissions` | 教师（课程管理权限）/ 学生（仅本人） | FR-LAB-03 |
| API-LAB-10 | LAB | 获取提交详情 | GET | `/api/labs/{labId}/submissions/{submissionId}` | 教师 / 提交者本人 | FR-LAB-03 |
| API-LAB-11 | LAB | 触发评测 | POST | `/api/labs/{labId}/submissions/{submissionId}/evaluate` | 教师、系统内部 | FR-LAB-04 |
| API-LAB-12 | LAB | 获取评测结果 | GET | `/api/labs/{labId}/submissions/{submissionId}/result` | 教师 / 提交者本人 | FR-LAB-04 |
| API-LAB-13 | LAB | 教师评分 | POST | `/api/labs/{labId}/submissions/{submissionId}/score` | 教师（课程管理权限） | FR-LAB-05 |
| API-LAB-14 | LAB | 获取实验统计 | GET | `/api/labs/{labId}/statistics` | 教师（课程管理权限） | FR-LAB-07 |
| API-LAB-15 | LAB | 管理测试用例 | POST/GET/PUT/DELETE | `/api/labs/{labId}/testcases` | 教师（课程管理权限） | FR-LAB-04 |
...
| DB-LAB-01 | LAB | `lab_experiment` | 实验表 | id, course_id, title, description, status, deadline, max_score, language, evaluation_mode, auto_evaluate, created_by | 存储实验基本信息和配置 |
| DB-LAB-02 | LAB | `lab_testcase` | 测试用例表 | id, lab_id, input, expected_output, score_weight, is_public, order_num, deleted | 存储实验测试用例 |
| DB-LAB-03 | LAB | `lab_submission` | 实验提交表 | id, lab_id, student_id, code_content, file_path, language, status, final_score, submitted_at | 存储学生提交记录 |
| DB-LAB-04 | LAB | `lab_evaluation` | 评测结果表 | id, submission_id, testcase_id, actual_output, passed, error_message, executed_at | 存储每个测试用例的评测结果 |
| DB-LAB-05 | LAB | `lab_score` | 评分记录表 | id, submission_id, teacher_id, score, comment, scored_at | 存储教师评分记录 |
...
| DB-HWK-07 | HWK | `t_hwk_judge_config` | 作业评测配置表 | id, homework_id, language_limit_json, time_limit_ms, memory_limit_kb, output_compare_mode, created_at, updated_at | 保存代码题统一评测配置，首版可简化为 homework 表中的 judge_config_id 引用 |
...
| 通知异常 | 通知事件格式不正确、站内消息写入失败 | 记录失败日志，业务主流程可按重要性决定是否回滚 | LRN |
...
| 成绩查询与统计 | 按课程、学生、成绩项、发布状态建立索引，统计分析可使用快照 | GRD |
...
| 4 | 单元测试 | LAB | 实验发布、实验提交、评测记录、教师评分、状态机 | TC-LAB-01 ~ TC-LAB-30 |
...
| FR-LAB-04 | 实验自动评测 | DSD-LAB-04 | UI-LAB-02 | API-LAB-11、API-LAB-12 | DB-LAB-02、DB-LAB-04 | TC-LAB-15 ~ TC-LAB-20 | 覆盖IO比对评测、测试用例管理、评测状态 |
| FR-LAB-05 | 教师评分与评语 | DSD-LAB-05 | UI-LAB-06 | API-LAB-13 | DB-LAB-05 | TC-LAB-21 ~ TC-LAB-24 | 覆盖教师评分、评语填写、成绩同步 |
| FR-LAB-06 | 实验结果展示 | DSD-LAB-06 | UI-LAB-07 | API-LAB-10、API-LAB-12 | DB-LAB-03、DB-LAB-04、DB-LAB-05 | TC-LAB-25 ~ TC-LAB-27 | 覆盖学生查看成绩、评语、评测详情 |
| FR-LAB-07 | 实验统计查询 | DSD-LAB-07 | UI-LAB-08 | API-LAB-14 | DB-LAB-03、DB-LAB-05 | TC-LAB-28 ~ TC-LAB-30 | 覆盖提交率、平均分、分数分布统计 |
| FR-LAB-08 | 实验报告管理 | DSD-LAB-08 | UI-LAB-02、UI-LAB-07 | API-LAB-08、API-LAB-10 | DB-LAB-03 | TC-LAB-31 ~ TC-LAB-32 | 覆盖实验报告上传与查看 |
| NFR-LAB-01 | 提交响应时间 | DSD-LAB-NFR-01 | UI-LAB-02 | API-LAB-08 | DB-LAB-03 | TC-LAB-N01 | 提交接口不阻塞等待评测结果 |
| NFR-LAB-02 | 评测执行时间 | DSD-LAB-NFR-02 | - | API-LAB-11 | DB-LAB-04 | TC-LAB-N02 | 单次评测不超过10秒 |
| NFR-LAB-03 | 数据隔离 | DSD-LAB-NFR-03 | UI-LAB-02、UI-LAB-05 | API-LAB-09、API-LAB-10 | DB-LAB-03 | TC-LAB-N03 | 学生仅可查看本人提交 |
| NFR-LAB-04 | 通知可靠性 | DSD-LAB-NFR-04 | UI-LAB-02 | API-LAB-13 | DB-LAB-05 | TC-LAB-N04 | 评分完成后触发LRN通知 |
| NFR-LAB-05 | 异常处理 | DSD-LAB-NFR-05 | UI-LAB-06 | API-LAB-11、API-LAB-13 | DB-LAB-02～DB-LAB-05 | TC-LAB-N05 | 评测失败允许教师手动处理 |
| FR-HW-01 | 作业创建与发布 | DS-HWK-01 | UI-HWK-01、UI-HWK-02、UI-HWK-03 | API-HWK-01、API-HWK-02、API-HWK-03、API-HWK-04、API-HWK-16、API-HWK-18 | DB-HWK-01、DB-HWK-02、DB-HWK-03、DB-HWK-07 | TC-HW-01、TC-HW-02、TC-HW-03 |  |
| FR-HW-02 | 学生作业查看与提交 | DS-HWK-02 | UI-HWK-04、UI-HWK-05 | API-HWK-05、API-HWK-06、API-HWK-07、API-HWK-17 | DB-HWK-01、DB-HWK-02、DB-HWK-04 | TC-HW-04、TC-HW-05、TC-HW-06 |  |
| FR-HW-03 | 提交历史管理 | DS-HWK-03 | UI-HWK-06 | API-HWK-08、API-HWK-09、API-HWK-10 | DB-HWK-04 | TC-HW-07、TC-HW-08 |  |
| FR-HW-04 | 自动评测 | DS-HWK-04 | UI-HWK-05、UI-HWK-07、UI-HWK-08 | API-HWK-07、API-HWK-11、API-HWK-12、API-HWK-18、API-HWK-19、API-HWK-20 | DB-HWK-03、DB-HWK-04、DB-HWK-05 | TC-HW-09、TC-HW-10、TC-HW-11、TC-HW-12 |  |
...
| --- | --- | --- | --- |
| R-01 | LAB 与 HWK 均包含自动评测能力，重复实现会增加维护成本 | LAB、HWK、GRD | 首版通过评测接口和状态枚举保持可复用边界，后续可抽取公共评测服务 |
| R-02 | GRD 依赖 LAB/HWK 来源成绩，来源状态不一致会影响成绩汇总 | LAB、HWK、GRD | 统一来源成绩 DTO、同步接口和更新时间字段，并保留成绩计算批次记录 |
| R-03 | 文件资源贯穿课程、实验和作业，存储策略变化会影响访问路径 | CRS、LAB、HWK | 首版采用文件存储抽象和元数据记录，业务表只保存文件标识或受控路径 |
