...
图 2-2 仅展示模块主依赖；成绩同步、通知触发和事件字段等联调细节以依赖表及各模块详细设计为准。
...
为保证详细设计在实现、测试和评审阶段具有一致口径，本文档对图示、编号和跨模块约束作如下约定。
...
7. 课程权限校验、文件存储、通知事件发布和成绩来源同步分别通过 `CoursePermissionClient`、`FileStorageService`、`NotificationEventPublisher`、`SourceGradePublisher` 四类代理接口完成。业务模块不得直接读取其他模块内部表，也不得重复实现权限、文件、通知和成绩同步逻辑。
...
| API-CRS-06 | 创建章节 | POST | /api/v1/courses/{courseId}/chapters | 课程教师 | FR-CR-03 |
| API-CRS-07 | 获取章节树 | GET | /api/v1/courses/{courseId}/chapters | 课程成员 | FR-CR-03 |
| API-CRS-08 | 修改章节 | PUT | /api/v1/courses/{courseId}/chapters/{chapterId} | 课程教师 | FR-CR-03 |
| API-CRS-09 | 删除章节 | DELETE | /api/v1/courses/{courseId}/chapters/{chapterId} | 课程教师 | FR-CR-03 |
| API-CRS-10 | 上传教学资源 | POST | /api/v1/courses/{courseId}/resources | 课程教师 | FR-CR-04 |
| API-CRS-11 | 获取资源列表 | GET | /api/v1/courses/{courseId}/resources | 课程成员 | FR-CR-04 |
| API-CRS-12 | 更新资源 | PUT | /api/v1/courses/{courseId}/resources/{resourceId} | 课程教师 | FR-CR-04 |
| API-CRS-13 | 删除资源 | DELETE | /api/v1/courses/{courseId}/resources/{resourceId} | 课程教师 | FR-CR-04 |
| API-CRS-14 | 学生选课 | POST | /api/v1/courses/{courseId}/join | 已登录用户 | FR-CR-02 |
...
| API-CRS-18 | 获取选课学生名单 | GET | /api/v1/courses/{courseId}/students | 课程教师、HWK/LAB/GRD模块 | FR-CR-05 |
| API-CRS-19 | 校验课程权限 | GET | /api/v1/courses/{courseId}/permissions/{userId} | HWK/LAB/GRD模块 | FR-CR-05 |
...
| 需求编号 | 需求名称 | 详细设计编号 | 页面编号 | API 编号 | 数据表编号 | 测试编号 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| FR-CR-01 | 课程创建与管理 | DSD-CRS-01 | UI-CRS-01, UI-CRS-02, UI-CRS-07 | API-CRS-01, API-CRS-03, API-CRS-18, API-CRS-19, API-CRS-20 | DB-CRS-01 | TC-CR-01 | 教师可创建、编辑、删除课程，自动成为课程创建者 |
| FR-CR-02 | 学生加入课程 | DSD-CRS-02 | UI-CRS-01 | API-CRS-02 | DB-CRS-04 | TC-CR-02 | 学生申请加入课程，教师审批后成为正式成员 |
| FR-CR-03 | 课程章节目录管理 | DSD-CRS-03 | UI-CRS-02, UI-CRS-03 | API-CRS-04, API-CRS-06, API-CRS-07, API-CRS-08 | DB-CRS-02 | TC-CR-03 | 教师创建、编辑、排序、删除章节，支持嵌套结构 |
| FR-CR-04 | 教学资源上传与管理 | DSD-CRS-04 | UI-CRS-04 | API-CRS-09, API-CRS-10, API-CRS-11 | DB-CRS-03 | TC-CR-04 | 教师上传资源，限制文件类型和大小，学生可下载 |
| FR-CR-05 | 课程成员关系维护 | DSD-CRS-05 | UI-CRS-05 | API-CRS-12, API-CRS-13, API-CRS-14 | DB-CRS-04 | TC-CR-05 | 教师管理课程成员，支持teacher、assistant、student三种角色 |
| FR-CR-06 | 课程公告发布与管理 | DSD-CRS-06 | UI-CRS-06 | API-CRS-15, API-CRS-16, API-CRS-17 | DB-CRS-05 | TC-CR-06 | 教师发布公告，支持置顶功能，成员实时接收通知 |
| NFR-CR-01 | 安全性 | DSD-CRS-NFR-01 | 全部 CRS 页面 | 全部 CRS 接口 | DB-CRS-01 ~ DB-CRS-05 | TC-CR-N01 | 课程数据权限控制、文件上传安全 |
| NFR-CR-02 | 可靠性 | DSD-CRS-NFR-02 | UI-CRS-01 ~ UI-CRS-07 | API-CRS-01 ~ API-CRS-22 | DB-CRS-01 ~ DB-CRS-05 | TC-CR-N02 | 课程数据一致性、成员关系正确性 |
| NFR-CR-03 | 可用性 | DSD-CRS-NFR-03 | UI-CRS-01 ~ UI-CRS-07 | API-CRS-01 ~ API-CRS-22 | - | TC-CR-N03 | 页面操作流畅、错误提示清晰 |
| NFR-CR-04 | 性能 | DSD-CRS-NFR-04 | UI-CRS-01、UI-CRS-04 | API-CRS-05、API-CRS-11 | DB-CRS-01 ~ DB-CRS-05 | TC-CR-N04 | 课程列表分页、资源上传大小限制 |
...
6. 通过 WebSocket 或轮询机制向学生端实时推送通知。
...
| FR-LN-04 | 通知分类推送与展示 | DSD-LRN-04   | UI-LRN-04 | API-LRN-06、API-LRN-09 | DB-LRN-04 | TC-LN-04 | 覆盖业务事件转通知 |
...
| NFR-LN-01 | 安全性 | DSD-LRN-NFR-01 | 全部 LRN 页面 | 全部 LRN 接口 | DB-LRN-01 ~ DB-LRN-07 | TC-LN-N01 | 数据隔离、事件接口鉴权、防刷限流 |
| NFR-LN-02 | 可靠性 | DSD-LRN-NFR-02 | UI-LRN-04 | API-LRN-09 | DB-LRN-04, DB-LRN-05 | TC-LN-N02 | 通知事件不丢失、WebSocket断线重连 |
| NFR-LN-03 | 可用性 | DSD-LRN-NFR-03 | UI-LRN-01 ~ UI-LRN-05 | API-LRN-01 ~ API-LRN-11 | - | TC-LN-N03 | 页面操作流畅、任务聚合结果可理解 |
| NFR-LN-04 | 性能 | DSD-LRN-NFR-04 | UI-LRN-01、UI-LRN-04 | API-LRN-01、API-LRN-06 | DB-LRN-01、DB-LRN-04 | TC-LN-N04 | 任务列表 ≤1.5秒、通知列表 ≤1秒 |
| NFR-LN-05 | 可测试性 | DSD-LRN-NFR-05 | 全部 LRN 页面 | 全部 LRN 接口 | 全部 LRN 表 | TC-LN-N05 | 提供Mock事件接口用于测试 |
...
| API-LAB-15 | 管理测试用例 | POST/GET/PUT/DELETE | /api/v1/labs/{labId}/testcases | 教师（课程管理权限） | FR-LAB-04 |
...
- **业务规则**：
  ...
  5. 若自动评测失败，按失败类型记录 COMPILE_ERROR、RUNTIME_ERROR、TIME_LIMIT_EXCEEDED、MEMORY_LIMIT_EXCEEDED 或 SYSTEM_ERROR，保留提交记录并允许教师重评或直接评分。
...
- **路径**：`/api/v1/labs/{labId}/testcases` 或 `/api/v1/labs/{labId}/testcases/{testcaseId}`
- **权限**：教师且拥有该课程管理权限
- **测试用例结构**：
...
7. 若执行过程中发生异常（编译错误、运行超时、内存超限、系统异常），标记评测状态并保留提交记录。
...
| --- | --- | --- |
| DRAFT | 草稿 | -> PUBLISHED |
| NOT_OPEN | 未开放 | -> PUBLISHED |
| PUBLISHED | 已发布（进行中） | -> CLOSED / SCORE_PUBLISHED |
| CLOSED | 已截止 | -> ARCHIVED |
...
| MEMORY_LIMIT_EXCEEDED | 内存超限 | 超过内存限制 |
...
| status | varchar(32) | NOT NULL, INDEX | PENDING | PENDING / RUNNING / ACCEPTED / WRONG_ANSWER / COMPILE_ERROR / RUNTIME_ERROR / TIME_LIMIT_EXCEEDED / MEMORY_LIMIT_EXCEEDED / SYSTEM_ERROR |
...
flowchart TD
  ...
  H -- 失败 --> M[记录 COMPILE_ERROR/RUNTIME_ERROR/TIME_LIMIT_EXCEEDED/MEMORY_LIMIT_EXCEEDED]
  K --> N{是否还有下一个用例?}
  L --> N
  M --> N
  N -- 是 --> E
...
stateDiagram-v2
    ...
    RUNNING --> MEMORY_LIMIT_EXCEEDED: 内存超限
    ...
    MEMORY_LIMIT_EXCEEDED --> SCORED: 教师完成评分
...
| RUNNING | ACCEPTED / WRONG_ANSWER / COMPILE_ERROR / RUNTIME_ERROR / TIME_LIMIT_EXCEEDED / MEMORY_LIMIT_EXCEEDED / SYSTEM_ERROR | 评测结束 | 写入评测记录和 auto_score |
...
stateDiagram-v2
    [*] --> DRAFT: 教师创建实验
    DRAFT --> NOT_OPEN: 设置未来开放时间
    DRAFT --> PUBLISHED: 教师立即发布实验
    NOT_OPEN --> PUBLISHED: 到达开放时间
...
| LAB-500-03 | 评测异常 | 运行时内存超限 | 评测状态置为 MEMORY_LIMIT_EXCEEDED，记录错误信息 | UI-LAB-02 |
...
| 评测超时控制 | Docker 沙箱超时（默认 60 秒）和内存限制 | 防止死循环或恶意代码占用资源，超限后记录明确评测状态 |
...
| FR-LAB-04 | 实验自动评测 | UI-LAB-02、UI-LAB-07 | API-LAB-11、API-LAB-12、API-LAB-15 | DB-LAB-02、DB-LAB-04 | TC-LAB-15 ~ TC-LAB-20 |
...
| FR-LAB-07 | 实验结果展示与学生反馈 | UI-LAB-07 | API-LAB-10、API-LAB-12、API-LAB-18 | DB-LAB-03、DB-LAB-04、DB-LAB-05、DB-LAB-06 | TC-LAB-28 ~ TC-LAB-30 |
| FR-LAB-08 | 实验统计与查询 | UI-LAB-08 | API-LAB-14 | DB-LAB-03、DB-LAB-04、DB-LAB-05、DB-LAB-06 | TC-LAB-31 ~ TC-LAB-33 |
...
| NFR-LAB-03 | 可追踪性 | 提交、评测、报告、评分和评分变更均保留时间、操作者和版本记录 | TC-LAB-N03 |
| NFR-LAB-04 | 安全性 | 当前用户由 JWT/认证上下文确定，隐藏用例不对学生开放，Docker 沙箱限制资源 | TC-LAB-N04 |
...
| TC-LAB-05 | 学生提交实验（正常） | 单元测试 | P0 | 创建提交记录，评测状态 PENDING |
| TC-LAB-06 | 学生在截止后提交 | 状态测试 | P0 | 返回 LAB-409-01 错误码 |
| TC-LAB-07 | 非课程成员提交实验 | 权限测试 | P0 | 返回 403 权限不足 |
| TC-LAB-08 | Docker 沙箱评测通过 | 单元测试 | P0 | 所有测试用例通过，evaluation_status=ACCEPTED，auto_score 正确 |
| TC-LAB-09 | Docker 沙箱评测输出错误 | 单元测试 | P0 | evaluation_status=WRONG_ANSWER，auto_score 按通过权重计算 |
| TC-LAB-10 | 评测超时处理 | 异常测试 | P1 | evaluation_status=TIME_LIMIT_EXCEEDED，记录超时错误信息 |
| TC-LAB-11 | 评测编译错误 | 异常测试 | P1 | evaluation_status=COMPILE_ERROR，记录编译错误信息 |
| TC-LAB-12 | 学生提交实验报告 | 单元测试 | P1 | 写入报告记录，版本递增，文件元数据正确 |
| TC-LAB-13 | 报告格式错误 | 参数测试 | P1 | 返回 LAB-400-06 错误码 |
| TC-LAB-14 | 教师评分（正常） | 单元测试 | P0 | 写入评分记录，触发通知并生成来源成绩 |
| TC-LAB-15 | 学生查看本人提交 | 权限测试 | P0 | 返回本人提交详情 |
| TC-LAB-16 | 学生查看他人提交 | 权限测试 | P0 | 返回 403 权限不足 |
| TC-LAB-17 | 教师评分超出范围 | 参数测试 | P0 | 返回 LAB-400-05 错误码 |
| TC-LAB-18 | 教师修改已评分成绩 | 单元测试 | P1 | 更新评分记录，记录变更日志 |
| TC-LAB-19 | 实验结果展示 | 系统测试 | P0 | 成绩发布前隐藏最终分，发布后显示最终成绩和评语 |
| TC-LAB-20 | 评测失败后教师重新评测 | 单元测试 | P1 | evaluation_status 从 SYSTEM_ERROR 回到 PENDING，并重新生成评测记录 |
| TC-LAB-31 | 实验统计查询 | 单元测试 | P1 | 返回提交率、未提交名单、评测完成率、平均分、分数分布和逾期提交数 |
| TC-LAB-32 | 评测失败后教师直接评分 | 单元测试 | P1 | 评测失败状态下仍可写入人工评分和最终分 |
| TC-LAB-33 | 学生多次提交版本递增 | 单元测试 | P1 | version 字段递增，历史记录保留 |
...
| API-CRS-06 | CRS | 创建章节 | POST | `/api/v1/courses/{courseId}/chapters` | 课程教师 | FR-CR-03 |
| API-CRS-07 | CRS | 获取章节树 | GET | `/api/v1/courses/{courseId}/chapters` | 课程成员 | FR-CR-03 |
| API-CRS-08 | CRS | 修改章节 | PUT | `/api/v1/courses/{courseId}/chapters/{chapterId}` | 课程教师 | FR-CR-03 |
| API-CRS-09 | CRS | 删除章节 | DELETE | `/api/v1/courses/{courseId}/chapters/{chapterId}` | 课程教师 | FR-CR-03 |
| API-CRS-10 | CRS | 上传教学资源 | POST | `/api/v1/courses/{courseId}/resources` | 课程教师 | FR-CR-04 |
| API-CRS-11 | CRS | 获取资源列表 | GET | `/api/v1/courses/{courseId}/resources` | 课程成员 | FR-CR-04 |
| API-CRS-12 | CRS | 更新资源 | PUT | `/api/v1/courses/{courseId}/resources/{resourceId}` | 课程教师 | FR-CR-04 |
| API-CRS-13 | CRS | 删除资源 | DELETE | `/api/v1/courses/{courseId}/resources/{resourceId}` | 课程教师 | FR-CR-04 |
| API-CRS-14 | CRS | 学生选课 | POST | `/api/v1/courses/{courseId}/join` | 已登录用户 | FR-CR-02 |
...
| API-CRS-18 | CRS | 获取选课学生名单 | GET | `/api/v1/courses/{courseId}/students` | 课程教师、HWK/LAB/GRD模块 | FR-CR-05 |
| API-CRS-19 | CRS | 校验课程权限 | GET | `/api/v1/courses/{courseId}/permissions/{userId}` | HWK/LAB/GRD模块 | FR-CR-05 |
...
| API-LAB-15 | LAB | 管理测试用例 | POST/GET/PUT/DELETE | `/api/v1/labs/{labId}/testcases` | 教师（课程管理权限） | FR-LAB-04 |
...
| DB-CRS-01 | CRS | `courses` | 课程表 | id, name, description, teacher_id, status, created_at, updated_at | 存储课程基本信息 |
| DB-CRS-02 | CRS | `chapters` | 章节表 | id, course_id, title, content, order_num, created_at | 存储课程章节目录 |
| DB-CRS-03 | CRS | `resources` | 资源表 | id, course_id, name, file_path, file_size, upload_user_id, created_at | 存储教学资源文件信息 |
| DB-CRS-04 | CRS | `course_members` | 课程成员表 | id, course_id, user_id, role, status, joined_at | 存储课程成员关系 |
| DB-CRS-05 | CRS | `announcements` | 公告表 | id, course_id, title, content, is_top, publish_user_id, created_at | 存储课程公告 |
| DB-LRN-01 | LRN | `lrn_learning_task` | 学习任务快照表 | id, user_id, course_id, source_module, source_id, task_type, title, deadline, progress, status, action_url, snapshot_at | 聚合任务的缓存表，定期刷新 |
...
| 6 | 单元测试 | GRD | 成绩项配置、成绩同步、成绩计算、成绩发布、复核申请 | TC-GR-01 ~ TC-GR-07 |
...
| FR-LN-05 | 通知触达与状态管理 | DSD-LRN-05 | UI-LRN-04 | API-LRN-07、API-LRN-08 | DB-LRN-05 | TC-LN-05 | 覆盖已读和删除留痕 |
| FR-LN-06 | 定时提醒与规则配置 | DSD-LRN-06 | UI-LRN-05 | API-LRN-10、API-LRN-11 | DB-LRN-06、DB-LRN-07 | TC-LN-06 | 覆盖提醒规则和通知偏好 |
| FR-LAB-01 | 实验创建与发布 | DSD-LAB-01 | UI-LAB-01、UI-LAB-04 | API-LAB-01、API-LAB-02、API-LAB-03、API-LAB-04、API-LAB-05、API-LAB-06、API-LAB-07 | DB-LAB-01、DB-LAB-02 | TC-LAB-01 ~ TC-LAB-07 | 覆盖实验创建、编辑、发布、截止、归档全生命周期 |
...
| FR-LAB-04 | 实验自动评测 | DSD-LAB-04 | UI-LAB-02、UI-LAB-07 | API-LAB-11、API-LAB-12、API-LAB-15 | DB-LAB-02、DB-LAB-04 | TC-LAB-15 ~ TC-LAB-20 | 覆盖 Docker 沙箱评测、测试用例管理、评测状态 |
...
| FR-LAB-07 | 实验结果展示与学生反馈 | DSD-LAB-07 | UI-LAB-07 | API-LAB-10、API-LAB-12、API-LAB-18 | DB-LAB-03、DB-LAB-04、DB-LAB-05、DB-LAB-06 | TC-LAB-28 ~ TC-LAB-30 | 覆盖发布前反馈控制、发布后成绩与评语展示 |
| FR-LAB-08 | 实验统计与查询 | DSD-LAB-08 | UI-LAB-08 | API-LAB-14 | DB-LAB-03、DB-LAB-04、DB-LAB-05、DB-LAB-06 | TC-LAB-31 ~ TC-LAB-33 | 覆盖提交率、未提交名单、评测完成率、平均分和分数分布 |
...
| NFR-LAB-03 | 可追踪性 | DSD-LAB-NFR-03 | UI-LAB-02、UI-LAB-05、UI-LAB-06 | API-LAB-09、API-LAB-10、API-LAB-13、API-LAB-16、API-LAB-17 | DB-LAB-03、DB-LAB-04、DB-LAB-05、DB-LAB-06、DB-LAB-07 | TC-LAB-N03 | 提交、评测、报告、评分和评分变更均留痕 |
| NFR-LAB-04 | 安全性 | DSD-LAB-NFR-04 | UI-LAB-02、UI-LAB-07 | API-LAB-09、API-LAB-10、API-LAB-12、API-LAB-18 | DB-LAB-02、DB-LAB-03、DB-LAB-04 | TC-LAB-N04 | 学生仅看本人提交，隐藏用例不开放，Docker 沙箱限制资源 |
