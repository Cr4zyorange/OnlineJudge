...
浏览器和前端代码继续只访问原有 `/api` 基址。Nginx 根据冻结的公开 API 路径选择 AUTH、CRS、Assessment、Learning & Grade 逻辑上游；未切流时四项均指向单体：
...
路由优先级先匹配课程下的实验/作业和成绩子路径，再匹配通用课程路径。AUTH 负责 `/api/v1/auth/**`、`/api/v1/users/me/**`、`/api/v1/admin/**`；Assessment 负责实验、作业、提交和评测；Learning & Grade 负责学习、通知、提醒和全部成绩资源；其余课程与章节路径归 CRS。精确根路径（例如 `/api/v1/courses`、`/api/v1/homeworks`、`/api/v1/notifications`）与带子路径形式均已覆盖。

网关只把浏览器的 `Authorization` 传给业务服务，并清空 `X-User-Id`、`X-Username`、`X-User-Role`、`X-Permissions`、`X-Course-Ids`、`X-Manageable-Course-Ids`，因此业务服务不得信任浏览器伪造的内部主体 Header。请求体上限为 55 MB；普通连接/读取/发送超时为 5/60/60 秒，Assessment 上传和提交读取/发送超时为 300 秒。代理重试显式关闭，避免非幂等请求重复提交。业务下游的 401/403/404 原样保留；连接失败和超时分别返回稳定的 `GATEWAY_502`、`GATEWAY_504` JSON，不包含内部主机名、堆栈或凭据。

Compose 首次启用生成配置挂载时，在仓库根目录执行：

```bash
mkdir -p tmp/gateway-runtime
cp deploy/nginx/gateway-defaults.env tmp/gateway-runtime/targets.env
set -a
. tmp/gateway-runtime/targets.env
set +a
./scripts/gateway/render-gateway-config.sh \
  --template deploy/nginx/gateway.conf.template \
  --output tmp/gateway-runtime/default.conf
docker compose -f deploy/docker/compose.yml -f deploy/docker/compose.gateway.yml \
  up -d --no-deps --force-recreate frontend
```

切流前先取得一个临时、最小权限的会话令牌，并在当前终端设置验证参数。令牌仅写入权限为 600 的临时 Header 文件，不会出现在标准输出；验证结束后文件自动删除：

```bash
export GATEWAY_MODE=compose
export GATEWAY_BASE=http://127.0.0.1:8088
export GATEWAY_BEARER_TOKEN='<temporary-session-token>'
export GATEWAY_SMOKE_PATH=/api/v1/auth/me
./scripts/gateway/switch-gateway-target.sh \
  --mode compose --service auth --target auth-service:8081
```

服务名只允许 `auth`、`crs`、`assessment`、`learning-grade`，目标只允许小写 `host:port`。每次命令只改变一个目标，重建 frontend 后依次检查公共健康、就绪和受保护 smoke。任一后置检查失败时会恢复 `targets.previous.env`、重新渲染并再次验证：切流成功退出 0；切流失败但回滚验证成功退出 1；回滚也无法验证退出 2。

显式回退单体使用同一命令，不需要编辑 Nginx 文件：

```bash
./scripts/gateway/switch-gateway-target.sh \
  --mode compose --service auth --target backend:8080
```

Kind 使用相同模板生成 `gateway-config` ConfigMap，并只读挂载到 frontend：
...
export GATEWAY_SMOKE_PATH=/api/v1/auth/me
./scripts/gateway/switch-gateway-target.sh \
  --mode kind --service auth --target auth-service:8081
```

切换其他服务时必须把 `GATEWAY_SMOKE_PATH` 改为该服务中当前会话有权访问的稳定只读接口。部署或回滚失败时执行 `scripts/kind/k8s-diagnose.sh`；诊断包只采集 `gateway-config`、容器内已挂载 Nginx 配置、`nginx -t`、工作负载状态和日志，不读取 Kubernetes Secret。
...
其中运行时测试会创建一次性私有网络、四个逻辑服务桩、单体兼容桩和真实 `nginx:1.27-alpine` 网关，覆盖路由、深链、multipart、身份 Header、401/403/404/502/504；退出时只清理本次随机命名的测试容器和网络。详细路由表与协作边界见 `docs/开发/D7-GATEWAY-路由切流与回滚.md`。
