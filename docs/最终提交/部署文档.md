...
保留既有 `mysql-data` 卷升级到包含 Issue #225 的版本时，在仓库根目录显式执行版本化迁移入口：

```bash
./database/mysql/apply-compose-migration.sh 20260822_01_add_hwk_statistics_attention_indexes.sql
```

该入口把 `database/migrations/` 下指定的版本化脚本交给 Compose MySQL 容器内的 `mysql` 客户端执行。`20260822_01_add_hwk_statistics_attention_indexes.sql` 会先检查当前库索引：旧库缺少两个索引时，通过一条原子 `ALTER TABLE` 同时建立；只有一个索引时补齐另一个；fresh Compose 或已完成升级的数据库再次执行时不改动 schema。因此失败后可安全重试，不需要删除持久化卷。生产或共享环境执行前仍应按常规流程备份数据库并保留命令输出。

演示账号和演示业务数据不是 MySQL 初始化脚本直接写入。后端启动后，AUTH 基础账号初始化器会补齐默认账号；`ONLINEJUDGE_DEMO_DATA_ENABLED=true` 时，集成演示数据初始化器会以 `ApplicationRunner` 方式检查 schema、账号和固定业务 ID，并对课程、学习任务、LAB/HWK、成绩和通知样例做幂等写入。保留数据卷重启时不会覆盖已有业务数据；如果此前关闭演示数据，之后重新启用并重启后端，缺失样例会在条件满足时补齐。学习行为记录遵守只追加原则：固定历史样例不会被改写；若演示学生在课程 `9501` 的近 7 天内没有资源访问记录，启动器会补充一条当前访问样例，确保学习仪表盘仍可用于验收，重复启动不会在同一统计窗口内重复补充。
