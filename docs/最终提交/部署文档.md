...
保留既有 `mysql-data` 卷升级到包含 Issue #225 和 Issue #222 的版本时，先同时备份数据库与 `onlinejudge_app-data` 卷，再在仓库根目录按版本号顺序执行迁移：

```bash
./database/mysql/apply-compose-migration.sh 20260822_01_add_hwk_statistics_attention_indexes.sql
./database/mysql/apply-compose-migration.sh 20260822_02_create_lab_submission_source_file.sql
```

该入口把 `database/migrations/` 下指定的版本化脚本交给 Compose MySQL 容器内的 `mysql` 客户端执行。`20260822_01_add_hwk_statistics_attention_indexes.sql` 会先检查当前库索引：旧库缺少两个索引时，通过一条原子 `ALTER TABLE` 同时建立；只有一个索引时补齐另一个。`20260822_02_create_lab_submission_source_file.sql` 在既有 LAB 实验和提交表之后建立 `lab_submission_source_file`，固化提交版本一对一、存储键唯一性、资产状态及 `lab_id/course_id/uploader_id/status` 四个单字段查询索引。fresh Compose 已通过 `compose-schema.sql` 包含两项变更，不需要额外执行。升级后应保留命令输出，且不得删除持久化卷来规避迁移失败。
...
恢复前应先确认目标环境可以接受覆盖现有数据；教学演示环境建议先 `down` 后恢复，再 `up -d`。DB-LAB-09 的 `storage_key` 与应用卷中的物理源文件是同一资产的两部分，因此数据库和 `onlinejudge_app-data` 必须使用同一备份窗口的配对产物恢复，禁止只恢复其中一份后就开放源文件下载。
