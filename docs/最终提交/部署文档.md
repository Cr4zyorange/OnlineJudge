...
### 5.9 Gateway 五服务入口、切流与回滚

浏览器和前端代码继续只访问 `/api`，但 Gateway 已从 frontend Nginx 中拆出，作为
`services/gateway/Dockerfile` 构建的独立 workload 运行。五个业务目标必须全部显式提供：
...
不再支持四服务分组、Learning/Grade 共用目标或 `backend:8080` 默认回退。Identity 负责
认证、当前用户和管理员公开接口；Course 负责课程和章节；Assessment 负责实验、作业、
提交和评测；Grade 负责成绩项、记录、发布、分析和复核；Learning 负责学习过程、通知和
提醒。课程下 Assessment/Grade 子资源先于通用 Course 路由匹配。`/internal/v2/**`、
...
使用 Compose overlay 前，#318 必须提供五个真实服务和 frontend 的同网络编排。只验证
配置合并时可执行：
...
export GATEWAY_SMOKE_PATH=/api/v1/grades
./scripts/gateway/switch-gateway-target.sh \
  --mode compose --service grade --target grade-canary:9084
```

服务名只允许 `identity`、`course`、`assessment`、`grade`、`learning`，目标只允许小写
`host:port`。状态文件必须恰好包含五个目标；旧四服务状态会被拒绝。脚本只改变一个目标，
重载独立 `gateway` 后检查健康和受保护 smoke。后置检查失败时恢复完整前一快照并重复验证：
成功退出 0；切换失败但回滚通过退出 1；回滚也无法验证退出 2。Kind 模式更新
`gateway-config` 并重启 #318 提供的 `deployment/gateway`，不再修改 frontend。
...
Docker Linux 引擎可用后运行 `bash scripts/gateway/tests/gateway-runtime.test.sh`，它创建五个
独立 upstream fixture 与 Gateway 镜像，覆盖五服务路由、任意身份 Header、request ID、
multipart、401/403/404/413/429/502/503/504 和非幂等无重试。引擎不可用时明确退出 69。
真实 Identity/Assessment 停机验签和凭据文件要求见
`docs/开发/D7-GATEWAY-路由切流与回滚.md`；完整五服务主链必须等待 #312、#339、#342
和 #318，fixture 绿色不能代替最终验收。
