...
> **D3 交付状态（2026-08-28）。** #287 数据库与 #289 容器实现已合入当前 `origin/dev`；#288 的 PR #303、#290 的 PR #298 尚未合入，#292 尚无 PR。当前段落和本节命令只描述已经存在的手工 Docker Compose 能力，不把 Kind、GitHub Actions 或端到端交付编排写成已实现或已验收。D3 的目标、变量、Secret、镜像版本和健康契约以 [D3 CI/CD 共享契约](../开发/D3-CICD-共享契约.md) 为准；面向干净环境的 Compose 入口、脚本索引和复演证据格式见仓库根目录 [README](../../README.md) 与 [submission/03_devops/README.md](../../submission/03_devops/README.md)。
