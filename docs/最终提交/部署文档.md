...
docker compose -f deploy/docker/compose.yml up -d --no-build --wait --wait-timeout 240
```

`build-images.sh` 会在执行 Docker 命令前拒绝缺失、非 40 位或不等于当前 `HEAD` 的 `GIT_SHA`，同时拒绝包含已跟踪或未跟踪改动的工作树，避免镜像内容与 OCI revision 指向的源码不一致；通过后只生成两个完整 SHA 镜像。需要一次性完成隔离烟测时，执行 `./scripts/docker/smoke-images.sh`；该脚本使用“SHA 前缀 + 单次运行标识”作用域的 Compose project、检查三服务 healthy、镜像引用/OCI revision/非 root 用户、后端 readiness、前端代理 readiness 和业务只读链路，失败时输出限定项目的状态与日志，最后只清理该烟测项目及其卷。
...
从历史 root 运行镜像升级到 #289 的非 root 后端镜像时，旧 `onlinejudge_app-data` 卷可能保留 root 所有权。完成备份并构建当前完整 SHA 镜像后，在启动新后端前执行一次所有权迁移；脚本只接受受限卷名、要求目标卷已存在，并在修改后以 UID 10001 验证可读写：
...
该步骤不删除或重建卷。自定义 Compose project 时先用 `docker volume ls` 确认实际 `app-data` 卷名，再通过 `APP_DATA_VOLUME` 显式传入；迁移失败不得继续启动新后端，也不得用 `down -v` 规避。
...
```bash
docker compose -f deploy/docker/compose.yml config >/tmp/oj-compose-config.txt
grep -n "client_max_body_size" deploy/nginx/default.conf
...
docker compose \
  ...
  -f deploy/docker/compose.eval.local.example.yml \
  up -d --build
```
...
docker compose \
  ...
  -f deploy/docker/compose.eval.yml \
  -f deploy/docker/compose.eval.local.example.yml \
  config >/tmp/oj-compose-eval-config.txt
...
```bash
docker compose -f deploy/docker/compose.yml -f deploy/docker/compose.eval.yml -f deploy/docker/compose.eval.local.example.yml exec backend \
  printenv ONLINEJUDGE_EVALUATION_SANDBOX_MODE
docker compose -f deploy/docker/compose.yml -f deploy/docker/compose.eval.yml -f deploy/docker/compose.eval.local.example.yml exec backend \
...
export GIT_SHA="$(git rev-parse HEAD)"
./scripts/docker/build-images.sh
ONLINEJUDGE_EVALUATION_SANDBOX_MODE=fake docker compose \
  -f deploy/docker/compose.yml up -d --no-build --wait --wait-timeout 240 \
...
本次最终交付提供可由本地、后续 Actions 和 Kubernetes 复用的固定镜像契约及手工 Docker Compose 入口：构建方使用完整 `GIT_SHA` 执行 `scripts/docker/build-images.sh`，部署方以相同变量和固定镜像名执行 Compose `--no-build` 启动，再按第 4 节完成接口和页面验收。本 Issue 不实现 Actions 或 Kubernetes 资源。
