...
`build-images.sh` 会在执行 Docker 命令前拒绝缺失、非 40 位或不等于当前 `HEAD` 的 `GIT_SHA`，并只生成两个完整 SHA 镜像。需要一次性完成隔离烟测时，执行 `./scripts/docker/smoke-images.sh`；该脚本使用 SHA 作用域的 Compose project、检查三服务 healthy、镜像引用/OCI revision/非 root 用户、后端 readiness、前端代理 readiness 和业务只读链路，失败时输出限定项目的状态与日志，最后只清理该烟测项目及其卷。
...
| 密钥和密码 | 通过部署 Secret 提供必填的 `MYSQL_PASSWORD`、`MYSQL_ROOT_PASSWORD`，并设置 `ONLINEJUDGE_NOTIFICATIONS_INTERNAL_TOKEN`；不得提交实际值或恢复演示默认值 |
