...
| Spring multipart 限制 | `application.properties` 默认单文件 `50MB`、单请求 `55MB`；`compose.yml` 可用环境变量覆盖 | 传输层上限高于 HWK 10MiB 业务值，因此 HWK 超限应由业务层返回 `HWK_4131`；如覆盖该值，不得降到 10MiB 以下 |
...
保留既有 `mysql-data` 卷升级到包含 Issue #225、Issue #222 和 Issue #214 的版本时，先同时备份数据库与 `onlinejudge_app-data` 卷，再在仓库根目录按版本号顺序执行迁移：
...
./database/mysql/apply-compose-migration.sh 20260822_03_create_hwk_submission_attachment.sql
```

该入口把 `database/migrations/` 下指定的版本化脚本交给 Compose MySQL 容器内的 `mysql` 客户端执行。`20260822_01` 补齐 HWK 统计/待处理索引，`20260822_02` 建立 LAB 源文件资产，`20260822_03` 在既有 HWK 提交表之后建立 `t_hwk_submission_attachment`，固化服务端 UUID/存储键唯一、单提交单附件、所有权、`UPLOADED/BOUND/DELETED` 状态与过期清理索引。fresh Compose 已通过 `compose-schema.sql` 包含三项变更，不需要额外执行。升级后应保留命令输出，且不得删除持久化卷来规避迁移失败。
