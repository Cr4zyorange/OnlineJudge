# D4 跨服务共享契约（四服务，v1 历史基线）

> **历史基线，不再是当前正本。** 本文件记录 #310 的四服务 v1 契约，保留给既有模块化单体的差异对照和迁移审计。五服务生产目标的唯一正本是 [#305 五服务架构冻结](D6-D7-五服务架构冻结-305.md)、[D6/D7 五服务共享契约（v2）](D6-D7-五服务共享契约-v2.md)及其 OpenAPI、AsyncAPI 和 ADR-006。任何新服务、adapter 或故障降级不得把本文件的进程内 SPI、`X-Internal-Token`、同步 callback 或内存 best-effort 投递作为生产路径。
>
...
| 超时 | `onlinejudge.integration.course.timeout-ms`（默认 1000ms）+ `max-pool-size`（默认 4）+ `queue-capacity`（默认 64，0=无缓冲）有界执行模型；超时/中断会取消后台 Provider 任务（`Future.cancel(true)`），饱和即拒绝/空名单（fail-closed）；非法配置值构造期快速失败；`@PreDestroy` 关闭执行器。 |
