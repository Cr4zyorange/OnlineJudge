...
> **历史基线，不再是当前正本。** 本文件记录 #310 的四服务 v1 契约，保留给既有模块化单体的差异对照和迁移审计。五服务生产目标的唯一正本是 [D6/D7 五服务共享契约（v2）](D6-D7-五服务共享契约-v2.md)，连同其 OpenAPI、AsyncAPI 和 ADR-006。任何新服务、adapter 或故障降级不得把本文件的进程内 SPI、`X-Internal-Token`、同步 callback 或内存 best-effort 投递作为生产路径。
>
